...
import {
  BookOpen, PenLine, Upload, RefreshCw,
...
export default function SignPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/sign/guidance/${token}`)
      .then(async (r) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/public/sign/guidance/${token}`, {
        method:  "POST",
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4" style={{ fontFamily: FONT }}>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 max-w-sm w-full p-6 text-center">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Thank you, {visit.name.split(" ")[0]}. Your Guidance Office visit record has been confirmed and signed.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8" style={{ fontFamily: FONT }}>
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">Guidance & Testing Center</p>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-gray-100" style={{ background: "#fef2f2" }}>
            <p className="text-sm font-bold" style={{ color: MAROON }}>{visit.name}</p>
            {visit.department && (
              <p className="text-[11px] text-gray-400">{visit.department}</p>
            )}
          </div>
          <div className="px-5 py-4 space-y-3">
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Date of Visit</p>
            ...
            <div>
              ...
              <p className="text-sm text-gray-700 leading-snug">{visit.purpose}</p>
            </div>
          </div>
        </div>
