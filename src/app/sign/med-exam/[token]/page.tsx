...
import {
  ClipboardList, PenLine, Upload, RefreshCw,
...
function SignaturePad({ onChange }: { onChange: (hasDrawn: boolean) => void }) {
  ...
  const move = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    lastPos.current = pos; onChange(true);
  };
...
export default function MedExamSignPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/sign/med-exam/${token}`)
      .then(async r => {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 1_500_000) { setUploadError("Image is too large. Please use a file under 1.5MB."); return; }
  ...
  const handleSubmit = async () => {
    ...
    if (activeTab === "draw") {
      ...
      if (!hasDrawn || !canvas) { setSubmitError("Please draw your signature first."); return; }
      signatureDataUrl = canvas.toDataURL("image/png");
    } else {
      if (!uploadPreview) { setSubmitError("Please upload your signature image first."); return; }
      signatureDataUrl = uploadPreview;
    }

    setSubmitting(true);
    try {
      const res = await fetch(`/api/public/sign/med-exam/${token}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ signatureDataUrl, method: activeTab === "upload" ? "uploaded" : "drawn" }),
      });
      const data = await res.json();
      if (!res.ok) { setSubmitError(data.error ?? "Failed to submit signature."); setSubmitting(false); return; }
      setSigned(true);
    } catch {
      setSubmitError("Network error. Please try again."); setSubmitting(false);
    }
  };
  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50" style={{ fontFamily: FONT }}>
      <div className="flex flex-col items-center gap-3 text-gray-300">
        ...
        <span className="text-sm font-medium">Loading exam record...</span>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4" style={{ fontFamily: FONT }}>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 max-w-sm w-full p-6 text-center">
        ...
        <p className="text-xs text-gray-500 leading-relaxed">
          Thank you, {exam.student.name.split(" ")[0]}. Your medical examination record has been confirmed and signed.
        </p>
      </div>
    </div>
  );
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-base font-black text-gray-900">Confirm Medical Exam Record</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-gray-100" style={{ background: "#fef2f2" }}>
            <p className="text-sm font-bold" style={{ color: MAROON }}>{exam.student.name}</p>
            <p className="text-[11px] text-gray-400 font-mono">{exam.student.studentNumber}</p>
          </div>
          <div className="px-5 py-4 space-y-3">
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Date of Examination</p>
              <p className="text-sm font-semibold text-gray-800">{fmtDate(exam.visitDate)}</p>
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Purpose</p>
              <p className="text-sm text-gray-700 leading-snug">{exam.purpose}</p>
            </div>
            {exam.remarks && (
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Remarks</p>
                <p className="text-sm text-gray-700 leading-snug">{exam.remarks}</p>
              </div>
            )}
            {exam.fitnessStatus && (
              <div>
                <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Medical Clearance</p>
                <p className="text-sm font-semibold" style={{ color: exam.fitnessStatus === "FIT" ? "#15803d" : "#b91c1c" }}>
                  {exam.fitnessStatus}
                </p>
                {exam.fitnessFor?.length > 0 && (
                  <p className="text-xs text-gray-500 mt-0.5">{exam.fitnessFor.join(", ")}</p>
                )}
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100">
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Please sign to confirm you received this medical examination record.</p>
          </div>

          <div className="flex gap-2 px-5 pt-4">
            {([["draw", "Draw", PenLine], ["upload", "Upload", Upload]] as const).map(([key, label, Icon]) => (
              <button key={key} type="button" onClick={() => setActiveTab(key)}
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-bold transition-all"
                style={activeTab === key ? { background: MAROON, color: "#fff" } : { background: "#f3f4f6", color: "#6b7280" }}>
                <Icon size={13} /> {label}
              </button>
            ))}
          </div>
          ...
          {submitError && (
            <div className="px-5 pb-1">
              <div className="text-xs font-semibold text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">{submitError}</div>
            </div>
          )}
          ...
          <div className="px-5 py-4">
            <button onClick={handleSubmit} disabled={submitting}
              ...
              {submitting ? <><RefreshCw size={14} className="animate-spin" /> Submitting...</> : <><Check size={14} /> Submit Signature</>}
            </button>
          </div>
        </div>
