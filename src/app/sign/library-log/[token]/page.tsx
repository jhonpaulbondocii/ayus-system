...
import { BookOpen, PenLine, Upload, RefreshCw, Check, X, AlertTriangle } from "lucide-react";
...
export default function LibraryLogSignPage() {
  ...
  useEffect(() => {
    if (!token) return;
    fetch(`/api/public/sign/library-log/${token}`)
      .then(async r => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await fetch(`/api/public/sign/library-log/${token}`, {
        method:  "POST",
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4" style={{ fontFamily: FONT }}>
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 max-w-sm w-full p-6 text-center">
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            Thank you, {log.name.split(",")[0]}. Your library card receiving log has been signed successfully.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-8" style={{ fontFamily: FONT }}>
      <div className="max-w-md mx-auto">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">University Library</p>
            <p className="text-base font-black text-gray-900">Library Card Receiving Log</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-gray-100" style={{ background: "#fef2f2" }}>
            <p className="text-sm font-bold" style={{ color: MAROON }}>{log.name}</p>
            {(log.courseYearSection || log.collegeDept) && (
              <p className="text-[11px] text-gray-400">{log.courseYearSection ?? log.collegeDept}</p>
            )}
          </div>
          <div className="px-5 py-4 space-y-3">
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Document Received</p>
              <p className="text-sm font-semibold text-gray-800">{log.documentReceived}</p>
            </div>
            <div>
              <p className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-0.5">Date Released</p>
              <p className="text-sm text-gray-700">{fmtDate(log.dateReceived)}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100">
            ...
            <p className="text-[11px] text-gray-400 mt-0.5">Please sign to confirm you received your library card.</p>
          </div>
