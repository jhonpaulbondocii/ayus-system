...
function MobileCourseCard({
  ...
  c: Course; onEdit: () => void; onDelete: () => void;
}) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-4 flex flex-col gap-3">
      <div className="flex items-start gap-3">
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button onClick={onEdit} className="w-7 h-7 flex items-center justify-center rounded-full text-gray-400 hover:bg-gray-100">
            <Pencil className="w-3.5 h-3.5" />
          </button>
          <button onClick={onDelete} className="w-7 h-7 flex items-center justify-center rounded-full text-gray-400 hover:bg-red-50 hover:text-red-500">
      ...
      <div className="flex items-center gap-3 flex-wrap text-xs text-gray-400">
        ...
        <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium border ml-auto
          ${c.status === "PUBLISHED" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-gray-100 text-gray-500 border-gray-200"}`}>
    ...
    <div className="p-4 sm:p-6 max-w-6xl">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-5 sm:mb-6">
        {[
          ...
          <div key={s.label} className={`bg-white border ${s.cls} rounded-xl px-4 sm:px-5 py-3 sm:py-4 ${i === 2 ? "col-span-2 sm:col-span-1" : ""}`}>
        ...
        {loading ? (
          ...
          <>
            ...
            {filtered.map(c => (
              <MobileCourseCard
                key={c.id} c={c}
                onEdit={() => setModal({ mode: "edit", id: c.id, form: { name: c.name, code: c.code, color: c.color, status: c.status, description: c.description ?? "", term: c.term ?? "" } })}
              ...
              <tr key={c.id} className="hover:bg-gray-50/50 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: c.color + "22" }}>
                ...
                <td className="px-5 py-3.5">
                  <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full
                    ${c.status === "PUBLISHED" ? "bg-emerald-50 text-emerald-700 border border-emerald-200" : "bg-gray-100 text-gray-500 border border-gray-200"}`}>
                    {c.status === "PUBLISHED" ? "Published" : "Unpublished"}
                  </span>
                </td>
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => setModal({ mode: "edit", id: c.id, form: { name: c.name, code: c.code, color: c.color, status: c.status, description: c.description ?? "", term: c.term ?? "" } })}
        ...
        <div className="fixed inset-0 z-300 flex items-end sm:items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:w-120 overflow-hidden max-h-[92vh] flex flex-col">
            ...
            <div className="flex gap-2 px-5 sm:px-6 py-4 border-t border-gray-100 bg-gray-50 shrink-0">
              <button onClick={() => setModal(null)} className="flex-1 py-2 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-100">
                Cancel
              </button>
              <button
                onClick={saveCourse}
        ...
        <div className="fixed inset-0 z-300 flex items-end sm:items-center justify-center bg-black/20 backdrop-blur-sm">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-xl p-6 w-full sm:w-80">
            ...
            <div className="flex gap-2">
              <button onClick={() => setDeleteTarget(null)} className="flex-1 py-2.5 border border-gray-200 rounded-lg text-sm text-gray-600 hover:bg-gray-50">
                Cancel
              </button>
              <button onClick={() => deleteCourse(deleteTarget.id)} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg">
                Delete
              </button>
            </div>
          </div>
