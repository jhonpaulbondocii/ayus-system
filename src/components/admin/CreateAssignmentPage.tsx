...
export default function CreateAssignmentPage() {
  ...
  // ── Read query params (name, points, group) ───────────────────────────────
  const initial = useMemo(() => ({
    name:   searchParams.get("name")   ?? "",
    points: searchParams.get("points") ?? "0",
    group:  searchParams.get("group")  ?? "",   // ← the group from URL
  }), [searchParams]);
  ...
  // ── group: start with URL param if present, else "Assignments" ────────────
  ...
  // ── Load assignment groups from API, then merge URL group param ───────────
  useEffect(()=>{
    if(!courseId) return;
    fetch(`/api/admin/courses/${courseId}/assignments`)
      ...
      .then(d=>{
        ...
        // Also include the group from the URL param so it appears in the dropdown
        const urlGroup = initial.group;
        if(urlGroup && !apiGroupNames.includes(urlGroup)) {
          apiGroupNames.push(urlGroup);
        }

        const builtGroups = apiGroupNames.map((n,i)=>({id:i+1,name:n}));
        setGroups(builtGroups);

        // Pre-select the URL group if valid, otherwise fallback to "Assignments"
        if(urlGroup && apiGroupNames.includes(urlGroup)) {
          setGroup(urlGroup);
        } else if(!urlGroup) {
  ...
  const handleSave=async(publish:boolean)=>{
    ...
    try{
      ...
      const res=await fetch(`/api/admin/courses/${courseId}/assignments`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({title:name.trim(),description,points:parseFloat(points)||0,submissionType,assignmentGroup:group,displayGradeAs,status:publish?"PUBLISHED":"UNPUBLISHED",assignees:row?.assignees??[],dueDate:row?.dueDate||null,dueTime:row?.dueTime||null,availableFrom:row?.availableFrom||null,availableFromTime:row?.availableFromTime||null,availableUntil:row?.until||null,untilTime:row?.untilTime||null,submissionEntries,submissionAttempts,allowedAttempts:submissionAttempts==="Limited"?allowedAttempts:null,doNotCount,isGroupAssignment,groupSetId:groupSet||null,notifyUsers})});
      if(res.ok){setPublished(publish);router.push(`/admin/courses/${courseId}/assignments`);}
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-[200px_1fr] items-start gap-y-4 gap-x-4 max-w-3xl">
              ...
              <div className="flex items-center gap-2">
                ...
                {/* Badge showing the pre-selected group from URL */}
              ...
              <>
                ...
                <div className="space-y-2 max-w-xl">
                  {submissionEntries.map((entry, idx) => (
                    <div key={entry.id}
                      className="border border-gray-200 rounded-sm p-3 space-y-2 relative bg-white">
