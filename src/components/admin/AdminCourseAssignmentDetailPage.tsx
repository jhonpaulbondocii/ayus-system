...
function DeleteConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40" onClick={onCancel}>
      <div className="bg-white rounded-xl shadow-2xl w-[380px] border border-gray-200 overflow-hidden"
...
export default function AdminCourseAssignmentDetailPage({
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/courses/${courseId}/sections`)
      .then(r=>r.json()).then(d=>{
        ...
        const sections: EnrolledUser[] = rawSections.map((s:{id:string;name:string})=>({
          id: `section:${s.id}`, name: s.name, courseRole: "Section",
        }));
        setEnrolledUsers([...staff,...sections]);
      }).catch(()=>{});
  },[courseId,assignmentId]);
  ...
  // ── Download submissions — grouped by student folder ──────────────────────
      ...
      for (const [userId, subs] of Object.entries(byStudent)) {
        ...
        for (let i = 0; i < subs.length; i++) {
          ...
          try {
            ...
            // Use original fileName if available, else derive from URL
  ...
  const toggleAssignee = (rowId:number,user:{id:string;label:string}) =>
    setAssignRows(p=>p.map(r=>{
      ...
      const next = has
        ? r.assignees.filter(a=>a.id!==user.id)
        : [...r.assignees.filter(a=>a.id!=="everyone"), user];
    ...
    <div className="flex flex-col h-full bg-white" style={{fontFamily:FONT}}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-8 py-6">
          ...
          {/* Title */}
          <h1 className="text-2xl font-black text-gray-900 mb-2">{assignment.title}</h1>
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <span className="text-xs text-gray-400">
                  Posted {fmtDateTime(creator.createdAt)}
                </span>
              </div>
            </div>
          )}
                  ...
                  <tr>
                    ...
                    <td className="px-5 py-3">
                      {forLabel === "Everyone" ? (
                        ...
                        <span className="text-sm font-bold cursor-default hover:underline"
                          style={{color:MAROON}}
        ...
        <div className="w-56 border-l border-gray-200 bg-white shrink-0 flex flex-col overflow-y-auto">
          ...
          <div className="px-4 py-4 space-y-3">
            ...
            {/* Download Submissions — grouped by student */}
            <button
            ...
            <button className="w-full flex items-center gap-2 text-xs font-bold hover:underline text-left transition-colors"
              style={{color:MAROON}}>
              <Upload size={13}/> Re-Upload Submissions
            </button>
          </div>

          {/* Submission count hint */}
      ...
      {showAssignPanel && (
        <>
          ...
          <div className="fixed right-0 top-0 h-full w-[340px] bg-white border-l border-gray-200 shadow-2xl z-50 flex flex-col" style={{fontFamily:FONT}}>
            ...
            <div className="mx-4 mt-4 mb-1 flex gap-2.5 rounded-xl px-3 py-2.5 border"
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-6">
                        ...
                        {row.assignees.map(a=>(
                          <span key={a.id} className="flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded-full text-white" style={{background:MAROON}}>
                            ...
                            <button type="button" tabIndex={-1}
                              ...
                              className="opacity-70 hover:opacity-100 leading-none font-black">×</button>
                          </span>
                        ))}
                        <input
                    ...
                    <div key={label}>
                      ...
                      <div className="flex gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <button onClick={()=>updateAssignRow(row.id,dateField,"")}
                            className="text-[10px] font-bold hover:underline shrink-0 transition-colors" style={{color:MAROON}}>
                            Clear
                          </button>
                        </div>
                      </div>
              ...
              <button onClick={addAssignRow}
                className="flex items-center gap-1.5 text-xs font-bold hover:underline transition-colors" style={{color:MAROON}}>
