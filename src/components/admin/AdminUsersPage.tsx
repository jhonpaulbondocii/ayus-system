...
interface Course {
  id: string; name: string; code: string; term: string | null;
}
...
function SimpleSelect({ value, onChange, options, placeholder, disabled }: {
...
function NonTeachingRankField({ value, onChange, existingRanks }: {
    ...
    <div className="space-y-1.5">
      {existingRanks.length > 0 && (
        <div className="flex gap-1.5">
          {(["text", "dropdown"] as const).map(m => (
            <button key={m} type="button"
              onClick={() => { setMode(m); onChange(""); }}
              className={`text-[10px] font-bold px-2 py-1 rounded-md border transition-all ${mode === m ? "text-white border-transparent" : "text-gray-500 border-gray-200 hover:border-gray-400"}`}
              style={mode === m ? { background: MAROON } : {}}>
              {m === "text" ? "New" : "Existing"}
            </button>
          ))}
        </div>
      ...
      {mode === "text" || existingRanks.length === 0
        ...
        : <SimpleSelect value={value} onChange={onChange} options={existingRanks} placeholder="Select existing rank…"/>}
    </div>
  );
}
...
function useCourses(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    fetch("/api/admin/courses").then(r => r.json())
      .then(d => { if (mounted) setCourses(d.courses ?? []); })
...
function GroupedCourseDropdown({ value, onChange, academic, nonAcademic, uncategorized, loading, placeholder, displayFn, keyFn }: {
  value: string; onChange: (v: string) => void;
      ...
      <div key={label}>
        <div className="px-2 pt-2.5 pb-1">
          <span className="inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest text-white px-2 py-0.5 rounded"
            style={{ background: MAROON }}>{icon}{label}</span>
        </div>
        {items.map(c => (
    ...
    <>
      <button ref={btnRef} type="button" onClick={handleOpen}
        ...
        {loading
          ...
          : <ChevronDown size={13} className="text-gray-400 shrink-0 ml-2"
...
const CreateUserModal = React.memo(function CreateUserModal({
  ...
  useEffect(() => {
    ...
    setStaffType(""); setAcademicRank(""); setError(""); setCopied(false);
  }, [isOpen]);

  useEffect(() => { setAcademicRank(""); }, [staffType]);

  const handleGenerate = useCallback(() => { setPassword(generatePassword()); setShowPassword(true); }, []);
  ...
  const fullName = buildFullName(firstName, middleName, lastName, true, suffix);
    ...
    try {
      const res  = await fetch("/api/admin/users", {
        ...
        body: JSON.stringify({ name: fullName, email: email.trim(), password,
          department: null, position: academicRank || null, accountType: staffType, status: "APPROVED" }),
      });
        ...
        <div className="px-5 sm:px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-xl p-3.5 space-y-3">
            ...
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2">
              ...
              <span className="text-sm font-semibold text-gray-700 truncate">
                {fullName || <span className="text-gray-300 italic">Full name preview</span>}
              </span>
            </div>
          </div>
          ...
          <Field label="Staff Type" required>
            <div className="flex gap-2">
              {(["Teaching", "Non-Teaching"] as const).map(t => (
                <button key={t} type="button" onClick={() => setStaffType(t)}
          ...
          {staffType === "Non-Teaching" && (
            <Field label="Academic Rank / Position">
              <NonTeachingRankField value={academicRank} onChange={setAcademicRank} existingRanks={existingNonTeachingRanks}/>
            </Field>
          )}
        ...
        <div className="px-5 sm:px-6 py-5 space-y-4">
          ...
          {success ? (
            ...
            <Field label="Select Course" required>
              <GroupedCourseDropdown value={selectedCourse} onChange={setSelectedCourse}
...
function MobileUserCard({ user, selected, onSelect, onView, onToggleDeactivate, onDelete, onEnroll }: {
  ...
  return (
    <div className={`bg-white rounded-xl border p-4 flex items-start gap-3 transition-colors
      ${selected ? "border-[#7b1113]/30 bg-red-50/20" : "border-gray-200"}`}>
      ...
      <button type="button" onClick={onView} className="flex-1 text-left min-w-0">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {user.position && (
            <span className="text-[10px] text-gray-400 truncate max-w-[120px]">{user.position}</span>
          )}
        </div>
      </button>
...
export default function AdminUsersPage() {
  ...
  const existingNonTeachingRanks = Array.from(new Set(
          ...
          <div className="px-4 sm:px-5 py-3 border-b border-gray-100 flex items-center gap-2 bg-white">
            ...
            <button type="button" onClick={() => setShowFilters(f => !f)}
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border text-[11px] font-bold transition-all shrink-0
                ${!(showFilters || deptFilter) ? "border-gray-200 text-gray-500 hover:border-gray-400 hover:text-gray-700" : ""}`}>
                  ...
                  return (
                    <button key={n} type="button" onClick={() => setPage(n)}
                      ...
                      className={`w-7 h-7 flex items-center justify-center rounded-lg text-[11px] font-semibold transition-all border
                        ${page !== n ? "border-gray-200 text-gray-500 hover:border-gray-400 hover:text-gray-700" : ""}`}>
                      {n}
                    </button>
                  );
                })}
