...
// src/components/admin/EditAssignmentPage.tsx
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded shadow-xl w-72 border border-gray-200" onClick={e=>e.stopPropagation()}>
        ...
        <div className="px-4 py-4 space-y-2 text-xs text-gray-700">
          {[["Words",words],["Characters (with spaces)",chars],["Characters (no spaces)",charsNoSpace],["Paragraphs",paragraphs]].map(([k,v])=>(
            <div key={k as string} className="flex justify-between border-b border-gray-100 pb-1 last:border-0">
...
function HTMLEditorModal({ html:init, onUpdate, onClose }: { html:string; onUpdate:(h:string)=>void; onClose:()=>void }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/40" onClick={onClose}>
      <div className="bg-white rounded shadow-xl w-[640px] border border-gray-200" onClick={e=>e.stopPropagation()}>
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200">
        ...
        <div className="p-4 grid grid-cols-5 gap-2">
          {colors.map(c=>(
            <div key={c} title={c}
              ...
              onClick={()=>{document.execCommand(type,false,c==="transparent"?null!:c);onClose();}}/>
          ))}
        </div>
      </div>
    </div>
...
function RichTextEditor({ onChange, placeholder="Start typing...", initialHtml="" }: {
  ...
  useEffect(()=>{
    if(!initialHtml||!editorRef.current||loadedRef.current) return;
  ...
  const menus=useMemo(():{ label:string;items:MItem[] }[]=>[
    ...
    {label:"Insert",items:[
      ...
      {type:"sub",icon:"▶",label:"Media",children:[
        {type:"action",label:"Insert/Edit Video",action:()=>{let url=prompt("Video URL (YouTube/Vimeo):");if(!url)return;if(url.includes("youtube.com/watch?v="))url=url.replace("watch?v=","embed/");else if(url.includes("youtu.be/"))url=url.replace("youtu.be/","www.youtube.com/embed/");else if(url.includes("vimeo.com/"))url=url.replace("vimeo.com/","player.vimeo.com/video/");insertHTML(`<iframe src="${url}" width="560" height="315" frameborder="0" allowfullscreen style="max-width:100%;border-radius:4px;"></iframe>`);}},
      ]},
    ...
    {label:"Tools",items:[
      ...
      {type:"sub",icon:"⊞",label:"Apps",children:[
        {type:"action",label:"Spell Checker",action:()=>alert("Use your browser's built-in spell checker (right-click for suggestions).")},
      ]},
    ...
    {label:"Table",items:[
      {type:"sub",icon:"⊞",label:"Table",picker:true,children:[],onPick:(r:number,c:number)=>insertTable(r,c)},
      {type:"sep"},
      {type:"sub",icon:"▣",label:"Cell",children:[
        {type:"action",label:"Insert cell before",action:()=>alert("Click inside a table cell first.")},
        {type:"action",label:"Insert cell after",action:()=>alert("Click inside a table cell first.")},
        {type:"action",label:"Delete cell",action:()=>alert("Click inside a table cell first.")},
        {type:"action",label:"Merge cells",action:()=>alert("Select cells to merge.")},
        {type:"action",label:"Cell properties",action:()=>alert("Click inside a cell to view properties.")},
      ]},
      {type:"sub",icon:"≡",label:"Row",children:[
        {type:"action",label:"Insert row before",action:()=>alert("Click inside a row first.")},
        {type:"action",label:"Insert row after",action:()=>alert("Click inside a row first.")},
        {type:"action",label:"Delete row",action:()=>alert("Click inside a row first.")},
        {type:"sep"},
        {type:"action",label:"Cut row",action:()=>exec("cut")},
        {type:"action",label:"Copy row",action:()=>exec("copy")},
      ]},
      {type:"sub",icon:"|||",label:"Column",children:[
        {type:"action",label:"Insert column before",action:()=>alert("Click inside a column first.")},
        {type:"action",label:"Insert column after",action:()=>alert("Click inside a column first.")},
        {type:"action",label:"Delete column",action:()=>alert("Click inside a column first.")},
      ]},
      {type:"sep"},
      {type:"action",label:"Table properties",action:()=>alert("Click inside a table first.")},
  ...
  const TBGroups=useMemo(()=>[
    ...
    [
      {label:"🔗",title:"Link",fn:()=>{const url=prompt("URL:");if(!url)return;const txt=prompt("Link text:")||url;insertHTML(`<a href="${url}">${txt}</a>`);}},
      {label:"🖼",title:"Image",fn:()=>{const url=prompt("Image URL:");if(!url)return;insertHTML(`<img src="${url}" style="max-width:100%;" />`);}},
    ],
    [
    ...
    [
      ...
      {label:"</>",title:"HTML editor",fn:()=>{setEditorHtml(editorRef.current?.innerHTML??"");setShowHTML(true);}},
      ...
      <div ref={wrapRef} className="border border-gray-300 rounded overflow-hidden flex flex-col" style={{minHeight:320}}>
        ...
        <div className="flex items-center gap-4 px-3 py-1 bg-[#f7f9fb] border-t border-gray-200 text-xs text-gray-400">
          ...
          <span className="ml-auto cursor-pointer hover:text-gray-600"
            onClick={()=>{setEditorHtml(editorRef.current?.innerHTML??"");setShowHTML(true);}} title="HTML Editor">
            &lt;/&gt;
          </span>
        </div>
      </div>
...
export default function EditAssignmentPage({
  ...
  // ── Assign ────────────────────────────────────────────────────────────────
  const [assignRows,     setAssignRows]     = useState<AssignTo[]>([{id:1,assignees:["Everyone"],dueDate:"",dueTime:"",availableFrom:"",availableFromTime:"",until:"",untilTime:""}]);
  ...
  useEffect(()=>{
    ...
    fetch(`/api/admin/courses/${courseId}/assignments/${assignmentId}`)
      ...
      .then(d=>{
        ...
        if(!a) return;
        setName(a.title??"");
        setPoints(String(a.points??0));
        setGroup(a.assignmentGroup??"Assignments");
        setDisplayGradeAs(a.displayGradeAs??"Points");
        setDoNotCount(a.doNotCount??false);
        setSubmissionType(a.submissionType??"Online");
        setPublished(a.status==="PUBLISHED");
        setSubmissionAttempts(a.submissionAttempts??"Unlimited");
        setAllowedAttempts(a.allowedAttempts??1);
        setIsGroupAssignment(a.isGroupAssignment??false);
        setNotifyUsers(a.notifyUsers??false);

        // Restore submission entries from onlineEntryOptions
        // Each entry was stored as "Label - Type" string
        const opts: string[] = a.onlineEntryOptions??[];
        if(opts.length>0){
          setSubmissionEntries(opts.map((opt,i)=>{
            const dashIdx=opt.lastIndexOf(" - ");
            if(dashIdx>-1){
              const label=opt.slice(0,dashIdx).trim();
              const type=opt.slice(dashIdx+3).trim();
              const validType=SUBMISSION_ENTRY_TYPES.includes(type)?type:"File Upload";
              return {id:i+1,label,required:false,type:validType};
            }
            return {id:i+1,label:opt,required:false,type:"File Upload"};
          }));
        }

        setAssignRows([{
          id:1,
          assignees:(a.assignees&&a.assignees.length>0)?a.assignees:["Everyone"],
          dueDate:           isoToDate(a.dueDate),
          dueTime:           isoToTime(a.dueDate),
          availableFrom:     isoToDate(a.availableFrom),
          availableFromTime: isoToTime(a.availableFrom),
          until:             isoToDate(a.availableUntil),
          untilTime:         isoToTime(a.availableUntil),
        }]);
        setInitialDesc(a.description??"");
        setLoading(false);
      })
  ...
  useEffect(()=>{if(!courseId)return;fetch(`/api/admin/courses/${courseId}/groupsets`).then(r=>r.json()).then(d=>setGroupSets(d.groupSets??[])).catch(()=>{});},[courseId]);
  useEffect(()=>{if(!courseId)return;fetch(`/api/admin/courses/${courseId}/assignments`).then(r=>r.json()).then(d=>{const list=d.assignments??[];const names:string[]=[...new Set<string>(list.map((a:{assignmentGroup:string})=>a.assignmentGroup||"Assignments"))];if(!names.includes("Assignments"))names.unshift("Assignments");setGroups(names.map((n,i)=>({id:i+1,name:n})));}).catch(()=>{});},[courseId]);
  useEffect(()=>{if(!courseId)return;fetch(`/api/admin/courses/${courseId}/sections`).then(r=>r.json()).then(d=>{setSections(d.sections??[]);setStaff(d.staff??[]);}).catch(()=>{});},[courseId]);
  useEffect(()=>{if(openDropdownId===null)return;const h=(e:MouseEvent)=>{if(!(e.target as HTMLElement).closest("[data-dropdown]"))setOpenDropdownId(null);};document.addEventListener("mousedown",h);return()=>document.removeEventListener("mousedown",h);},[openDropdownId]);
  ...
  const updateAssignRow = (id:number,field:keyof AssignTo,value:string|string[])=>setAssignRows(p=>p.map(r=>r.id===id?{...r,[field]:value}:r));
  ...
  const handleSave=async(publish:boolean)=>{
    ...
    try{
      ...
      if(res.ok){setPublished(publish);router.push(`/admin/courses/${courseId}/assignments/${assignmentId}`);}
  ...
  const BottomBar=(
    <div className="shrink-0 border-t border-gray-200 bg-white px-8 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2">
        <button onClick={()=>router.push(`/admin/courses/${courseId}/assignments/${assignmentId}`)} disabled={saving}
          className="h-8 px-5 border border-gray-300 bg-white text-xs text-gray-700 rounded hover:bg-gray-50 disabled:opacity-50">
          Cancel
        </button>
        {activeTab!=="details"&&(
          <button type="button"
            onClick={()=>setActiveTab(activeTab==="submission"?"details":activeTab==="settings"?"submission":"settings")}
            className="h-8 px-5 border border-gray-300 bg-white text-xs text-gray-700 rounded hover:bg-gray-50">
            ← Back
          </button>
        )}
        {activeTab!=="assign"&&(
          <button type="button"
            ...
            className="h-8 px-5 border border-gray-300 bg-gray-50 text-xs text-gray-700 rounded hover:bg-gray-100">
            Next →
          </button>
        )}
        {activeTab==="assign"&&(
    ...
    <div className="w-full h-full bg-white flex flex-col" suppressHydrationWarning>
      ...
      <div className="flex items-center justify-between px-6 py-2.5 border-b border-gray-200 bg-white shrink-0">
        <div className="flex items-center gap-2 text-xs text-gray-500">
          <span>Editing:</span>
          <span className="font-semibold text-gray-700 truncate max-w-xs">{name||"Assignment"}</span>
        </div>
      ...
      <div className="flex items-end border-b border-gray-200 px-6 bg-white shrink-0">
        {TABS.map(t=>(
          <button key={t.key} onClick={()=>setActiveTab(t.key)}
            className={`px-5 py-2 text-xs border border-b-0 -mb-px mr-0.5 rounded-t transition-colors
              ${activeTab===t.key
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-[200px_1fr] items-start gap-y-4 gap-x-4 max-w-3xl">
              ...
              <input type="number" min={0} value={points} onChange={e=>setPoints(e.target.value)}
                className="h-8 border border-gray-300 rounded-sm px-3 text-xs w-80 outline-none focus:border-[#7b1113]"/>
              ...
              <select value={group} onChange={e=>{if(e.target.value==="__create__"){setNewGroupName("");setGroupModalOpen(true);}else setGroup(e.target.value);}}
                className="h-8 border border-gray-300 rounded-sm px-3 text-xs w-80 bg-white outline-none focus:border-[#7b1113]">
        ...
        {activeTab==="submission"&&(
          <div className="grid grid-cols-[200px_1fr] items-start gap-y-5 gap-x-4 max-w-3xl">
            ...
            {submissionType==="Online"&&(
              <>
                <label className="text-xs text-gray-700 text-right pt-2">
                  ...
                  {submissionEntries.map((entry,idx)=>(
                    <div key={entry.id} className="border border-gray-200 rounded-sm p-3 space-y-2 relative bg-white">
                      {submissionEntries.length>1&&(
                        <button type="button" onClick={()=>removeSubmissionEntry(entry.id)}
                          className="absolute top-2 right-2 text-gray-400 hover:text-red-500 text-xs leading-none">✕</button>
                      )}
                      ...
                      <label className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer pt-1">
                        <input type="checkbox" checked={entry.required}
              ...
              {isGroupAssignment&&(
                <div className="pl-2 space-y-2">
                  ...
                  <div>
                    ...
                    <select value={groupSet}
                      onChange={e=>{if(e.target.value==="__new__"){setGroupSetName("");setSelfSignUp(false);setRequireSameSection(false);setGroupStructure("Create groups later");setShowGroupSetModal(true);}else setGroupSet(e.target.value);}}
          ...
          <div className="grid grid-cols-[200px_1fr] items-start gap-y-5 gap-x-4 max-w-3xl">
            ...
            <div className="space-y-3">
              ...
              <button type="button" onClick={addAssignRow}
                className="w-full max-w-xl h-8 border border-gray-300 bg-gray-50 text-xs text-gray-600 rounded-sm hover:bg-gray-100 flex items-center justify-center gap-1">
                + Assign To
              </button>
            </div>
            <div/>
          ...
          <div className="w-[460px] bg-white shadow-xl border border-gray-200 rounded">
            ...
            <div className="px-6 py-6">
              <div className="flex items-center gap-3">
                ...
                <input value={newGroupName} onChange={e=>setNewGroupName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&saveGroup()} placeholder="e.g., Essay Group 1"
      ...
      {showGroupSetModal&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
          <div className="bg-white rounded-lg shadow-2xl w-[520px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-0">
              <div className="flex items-center gap-4 pb-4">
                ...
                <input value={groupSetName} onChange={e=>setGroupSetName(e.target.value)} placeholder="Enter Group Set Name"
                  className="flex-1 h-9 border border-gray-300 rounded px-3 text-sm outline-none focus:border-[#7b1113]"/>
              </div>
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    {selfSignUp&&(
                      <div className="pt-2 space-y-3">
                        <div><p className="text-sm text-gray-700 mb-1">Create groups now</p>
                        ...
                        <div><p className="text-sm text-gray-700 mb-1">Limit group members to</p>
                          <div className="flex items-center border border-gray-300 rounded w-32">
              ...
              {!selfSignUp&&(
                <div className="border-t border-gray-200 py-4">
                  <div className="flex items-start gap-4">
                    <label className="text-sm text-gray-700 w-36 shrink-0 pt-2">Group Structure</label>
                    <select value={groupStructure} onChange={e=>setGroupStructure(e.target.value)}
                      className="flex-1 h-9 border border-gray-300 rounded px-3 text-sm bg-white outline-none focus:border-[#7b1113]">
              ...
              {groupStructure!=="Create groups later"&&(
                <div className="border-t border-gray-200 py-4">
                  <div className="flex items-start gap-4">
                    <label className="text-sm text-gray-700 w-36 shrink-0 pt-0.5">Leadership</label>
                    <div className="space-y-2 flex-1">
                      <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                      ...
                      <div className="pl-2 space-y-1">
                        {[["first","Set first student to join as group leader"],["random","Set a random student as group leader"]].map(([val,lbl])=>(
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 bg-gray-50 rounded-b-lg">
              ...
              <button onClick={async()=>{
                ...
                try{const res=await fetch(`/api/admin/courses/${courseId}/groupsets`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:groupSetName.trim(),selfSignUp,requireSameSection,groupStructure,createGroupsNow,limitGroupMembers,autoAssignLeader,leaderType})});
                if(res.ok){const data=await res.json();const newGs=data.groupSet;setGroupSets(p=>[...p,{id:newGs.id,name:newGs.name}]);setGroupSet(newGs.id);setShowGroupSetModal(false);}}catch{/*ignore*/}
