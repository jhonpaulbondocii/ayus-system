...
function PublisherAvatar({ name, image, size = 20 }: { name?: string | null; image?: string | null; size?: number }) {
  ...
  if (image && !imgErr)
    return (
      <Image src={image} alt={name ?? ""} width={size} height={size} onError={() => setImgErr(true)}
...
function PublisherChip({ name, image, role }: { name?: string | null; image?: string | null; role?: string | null }) {
  ...
  return (
    <span className="flex items-center gap-1 text-[11px] text-gray-500">
      ...
      {role && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase" style={{ background: "#eff6ff", color: "#1d6fa4", border: "1px solid #bfdbfe" }}>{role}</span>}
    </span>
  );
}
...
function AuthorBadge({ name, role }: { name: string; role: string }) {
  return (
    <span className="inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full border" style={{ background: "#fdf8f8", color: MAROON, borderColor: "#f0c0c0" }}>
...
function PublishToggle({ published, onToggle }: { published: boolean; onToggle: () => void }) {
  return (
    <button type="button" onClick={onToggle} title={published ? "Published — click to unpublish" : "Unpublished — click to publish"}
      ...
      {published ? (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="9" fill="#16a34a"/><path d="M5.5 10.5l3 3 6-6" stroke="#fff" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
      ) : (
        <svg width="20" height="20" viewBox="0 0 20 20" fill="none"><circle cx="10" cy="10" r="9" stroke="#9ca3af" strokeWidth="1.5" fill="none"/><line x1="6" y1="14" x2="14" y2="6" stroke="#9ca3af" strokeWidth="1.5" strokeLinecap="round"/></svg>
      )}
    </button>
  );
}
...
// ── Row 3-dot menu ────────────────────────────────────────────────────────────
function FormRowMenu({ formId, onView, onEdit, onDelete }: {
  ...
  const handleOpen = () => {
    ...
    setStyle({ position: "fixed", top, left, zIndex: 9999, background: "#fff", border: "1px solid #e5e7eb", borderRadius: 8, boxShadow: "0 4px 16px rgba(0,0,0,.12)", minWidth: w, overflow: "hidden" });
    setOpen(v => !v);
  };

  const menuItems = [
    { label: "View", icon: <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>, action: () => { setOpen(false); onView(formId); } },
    { label: "Edit", icon: <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>, action: () => { setOpen(false); onEdit(formId); } },
    { label: "Delete", danger: true, icon: <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth={1.8} viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg>, action: () => { setOpen(false); onDelete(formId); } },
  ];

  return (
    <>
      <button ref={btnRef} type="button" onClick={handleOpen} className="w-8 h-8 flex items-center justify-center rounded hover:bg-gray-100 text-gray-500">
...
function DeleteFormModal({ title, onClose, onConfirm, deleting }: { title: string; onClose: () => void; onConfirm: () => void; deleting: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 px-0 sm:px-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full sm:max-w-md border border-gray-200 overflow-hidden" onClick={e => e.stopPropagation()} style={{ fontFamily: FONT }}>
        ...
        <div className="px-5 py-5">
          <p className="text-sm text-gray-700">Are you sure you want to delete <strong>&ldquo;{title}&rdquo;</strong>? This action cannot be undone.</p>
        </div>
        <div className="flex items-center justify-end gap-2 px-5 py-4 bg-gray-50 border-t border-gray-200">
          <button onClick={onClose} disabled={deleting} className="h-10 px-5 border border-gray-300 text-sm text-gray-600 rounded-xl hover:bg-gray-100 disabled:opacity-50">Cancel</button>
          <button onClick={onConfirm} disabled={deleting} className="h-10 px-5 text-sm text-white rounded-xl hover:opacity-90 disabled:opacity-50 font-bold" style={{ background: "#dc2626" }}>{deleting ? "Deleting..." : "Delete"}</button>
        </div>
      </div>
    </div>
  );
...
// ── Form Card (mobile) / Form Row (desktop) ───────────────────────────────────
function FormRow({ form, variant, onView, onEdit, onDelete, onTogglePublish }: {
  form: Form; variant: "mine" | "others";
  onView: (id: string | number) => void;
  onEdit: (id: string | number) => void;
  onDelete: (id: string | number) => void;
  ...
  return (
    <div className="relative border-b border-gray-100 last:border-0 transition-colors hover:bg-gray-50"
      ...
      {/* Mobile layout */}
      <div className="sm:hidden flex flex-col gap-2 px-4 py-3 pl-5">
        <div className="flex items-start gap-2">
          <div className="mt-0.5 shrink-0">
            <PublishToggle published={form.published} onToggle={() => onTogglePublish(form.id)}/>
          </div>
          <div className="flex-1 min-w-0">
            <button onClick={() => onView(form.id)} className="text-sm font-semibold text-left hover:underline w-full" style={{ color: MAROON }}>
              {form.title}
            </button>
            <div className="flex items-center gap-1.5 flex-wrap mt-1">
              <span className="text-[10px] px-2 py-0.5 rounded-full text-white font-medium" style={{ background: typeColors[form.formType] ?? MAROON }}>{form.formType}</span>
        ...
        <div className="flex gap-2 pl-7">
          <button onClick={() => onView(form.id)} className="flex-1 py-1.5 text-[11px] font-semibold border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">View</button>
          <button onClick={() => onEdit(form.id)} className="flex-1 py-1.5 text-[11px] font-semibold border border-gray-200 rounded-lg text-gray-600 hover:bg-gray-50">Edit</button>
          <button onClick={() => onDelete(form.id)} className="flex-1 py-1.5 text-[11px] font-semibold border border-red-100 rounded-lg text-red-500 hover:bg-red-50">Delete</button>
        </div>
      </div>

      {/* Desktop layout */}
      <div className="hidden sm:flex items-center gap-3 px-4 py-3.5 pl-5">
        <div className="shrink-0">
          <PublishToggle published={form.published} onToggle={() => onTogglePublish(form.id)}/>
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-sm font-semibold cursor-pointer hover:underline truncate" style={{ color: MAROON }} onClick={() => onView(form.id)}>{form.title}</h3>
            <span className="text-[10px] px-2 py-0.5 rounded-full text-white font-medium shrink-0" style={{ background: typeColors[form.formType] ?? MAROON }}>{form.formType}</span>
            {!form.published && <span className="text-[10px] text-amber-600 font-medium">Not Published</span>}
          </div>
...
function OthersAuthorSection({ authorName, authorRole, authorImage, items, onView, onEdit, onDelete, onTogglePublish }: {
  authorName: string; authorRole?: string | null; authorImage?: string | null; items: Form[];
  onView: (id: string | number) => void; onEdit: (id: string | number) => void;
  onDelete: (id: string | number) => void; onTogglePublish: (id: string | number) => void;
}) {
  const [collapsed, setCollapsed] = useState(false);
  return (
    <div className="mb-3 sm:mb-4">
      <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 border select-none cursor-pointer"
        style={{ background: "#eff6ff", borderColor: "#bfdbfe" }} onClick={() => setCollapsed(c => !c)}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#1d6fa4" strokeWidth="2.5"
          style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform .15s", flexShrink: 0 }}><path d="M6 9l6 6 6-6"/></svg>
        <PublisherAvatar name={authorName} image={authorImage} size={20}/>
        <span className="text-xs sm:text-sm font-semibold truncate flex-1" style={{ color: "#1d4ed8" }}>{authorName}</span>
        {authorRole && <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase hidden sm:inline" style={{ background: "#eff6ff", color: "#1d6fa4", border: "1px solid #bfdbfe" }}>{authorRole}</span>}
        <span className="text-xs text-blue-400 shrink-0">({items.length})</span>
      </div>
      {!collapsed && (
        <div className="border border-t-0" style={{ borderColor: "#bfdbfe" }}>
          {items.map(f => <FormRow key={f.id} form={f} variant="others" onView={onView} onEdit={onEdit} onDelete={onDelete} onTogglePublish={onTogglePublish}/>)}
        </div>
      )}
    </div>
  );
...
function OthersTypeSection({ typeName, items, onView, onEdit, onDelete, onTogglePublish }: {
  ...
  return (
    <div className="mb-3 sm:mb-4">
      <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 border select-none cursor-pointer"
        style={{ background: "#f0f9ff", borderColor: "#bae6fd" }} onClick={() => setCollapsed(c => !c)}>
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#0369a1" strokeWidth="2.5"
          style={{ transform: collapsed ? "rotate(-90deg)" : "rotate(0deg)", transition: "transform .15s", flexShrink: 0 }}><path d="M6 9l6 6 6-6"/></svg>
        <span className="text-[10px] px-2 py-0.5 rounded-full text-white font-medium" style={{ background: typeColors[typeName] ?? MAROON }}>{typeName}</span>
...
export default function AdminCourseFormsPage({ courseId, currentUserId, currentUserName }: Props) {
  ...
  const handleView   = (id: string | number) => router.push(`/admin/courses/${courseId}/forms/${id}`);
  const handleEdit   = (id: string | number) => router.push(`/admin/courses/${courseId}/forms/${id}/edit`);
  ...
  return (
    <div className="bg-white" style={{ fontFamily: FONT }}>
      ...
      {/* Toolbar */}
      <div className="flex items-center justify-between px-3 sm:px-8 py-2.5 border-b border-gray-100 gap-2">
        <div className="relative flex-1 sm:flex-none">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400"/>
          <input value={mySearch} onChange={e => setMySearch(e.target.value)} placeholder="Search your forms..."
            className="pl-8 pr-3 py-1.5 border rounded-lg text-sm w-full sm:w-52 focus:outline-none" style={{ borderColor: "#d1d5db" }}/>
        </div>
        <button onClick={() => router.push(`/admin/courses/${courseId}/forms/new`)} style={{ background: MAROON }}
          className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-semibold text-white rounded-lg hover:opacity-90 shrink-0 whitespace-nowrap">
          <Plus size={14}/> New Form
        </button>
      </div>

      {/* My Forms list */}
      <div className="px-3 sm:px-5 py-3 border-b-2 border-gray-200">
        <div className="border border-gray-200 rounded-xl overflow-hidden">
          <div className="flex items-center gap-2 px-4 py-3 bg-gray-50 border-b border-gray-200 cursor-pointer select-none hover:bg-gray-100"
            ...
            <span className="text-xs text-gray-400">{myExpanded ? "▾" : "▸"}</span>
            <span className="text-sm font-medium text-gray-700">My Forms</span>
            {myForms.length > 0 && <span className="text-xs text-gray-400">({myForms.length})</span>}
          </div>
          {myExpanded && (
            <div>
              {myFiltered.length === 0 && myForms.length === 0 ? (
                ...
                myFiltered.map(f => (
                  <FormRow key={f.id} form={f} variant="mine" onView={handleView} onEdit={handleEdit}
                    onDelete={id => setDeleteTarget(forms.find(x => x.id === id) ?? null)} onTogglePublish={handleTogglePublish}/>
                ))
              )}
            </div>
          )}
      ...
      <div className="flex items-center gap-2 px-4 sm:px-8 py-2.5 border-b border-t" style={{ color: "#1d6fa4", background: "#eff6ff", borderColor: "#bfdbfe" }}>
        ...
        {otherForms.length > 0 && <span className="ml-1 font-normal normal-case text-blue-400 text-xs">({otherForms.length})</span>}
      </div>

      {/* Others toolbar */}
      <div className="flex items-center justify-between px-3 sm:px-8 py-2.5 border-b border-gray-100 gap-2 flex-wrap">
        <div className="relative flex-1 sm:flex-none">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400"/>
          <input value={othersSearch} onChange={e => setOthersSearch(e.target.value)} placeholder="Search others' forms..."
            className="pl-8 pr-3 py-1.5 border rounded-lg text-sm w-full sm:w-52 focus:outline-none" style={{ borderColor: "#d1d5db" }}/>
        </div>
        <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden shrink-0">
          {(["author", "type"] as const).map(mode => (
            <button key={mode} onClick={() => setOthersViewMode(mode)}
              className="px-3 py-1.5 text-xs font-bold border-none transition-colors capitalize"
              style={othersViewMode === mode ? { background: MAROON, color: "#fff" } : { background: "transparent", color: "#6b7280" }}>
              {mode === "type" ? "By Type" : "By Author"}
            </button>
          ))}
        </div>
      </div>
      ...
      {/* Others list */}
      <div className="px-3 sm:px-5 py-3 pb-8">
        {otherForms.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center" style={{ background: "#eff6ff" }}>
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#1d6fa4" strokeWidth="1.5"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
            </div>
      ...
      {/* Delete modal */}
      {deleteTarget && (
        <DeleteFormModal title={deleteTarget.title} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} deleting={deleting}/>
      )}
    </div>
  );
}