...
function FilterPanel({
  ...
  useEffect(() => {
    if (!open) {
      setTimeout(() => {
        setSection("root"); setStartDate(""); setEndDate(""); setDateError("");
      }, 0);
    }
  }, [open]);
  ...
  const OptionButton = ({
    type, value, label,
  }: { type: ActiveFilter["type"]; value: string; label?: string }) => {
    ...
    <div ref={ref}
      ...
      {section === "studentGroups" && (
        <div>
          <BackButton to="root" onNavigate={setSection} />
          <div className="px-4 py-2 border-b border-gray-100">
            <p className="text-xs font-black text-gray-700">Staff Groups</p>
          </div>
          <div className="max-h-64 overflow-y-auto py-1">
...
function GradePanel({
  ...
  const [pointsInput, setPointsInput] = useState(
    panel.grade.grade !== null ? String(panel.grade.grade) : ""
  );
  ...
  const handleSave = async () => {
    ...
    if (!isCI && grade !== null) {
      if (grade < 0 || grade > panel.maxPoints) {
        setError(`Score must be between 0 and ${panel.maxPoints}.`);
        return;
      }
    }
    setSaving(true);
          ...
          <div className="px-5 py-4 border-b border-gray-100">
            ...
            {isCI && (
              <div className="relative">
                <select
                  value={ciValue}
                  onChange={e => setCiValue(e.target.value as "complete" | "incomplete" | "ungraded")}
                ...
                <div className="flex items-center gap-2">
                  ...
                  {pctInput !== "" && !isNaN(parseFloat(pctInput)) && (
                    <div className="h-10 px-3 rounded-xl flex flex-col items-center justify-center shrink-0 border"
                      ...
                      {letterGrade !== "—" && (
                        <span className="text-[9px] font-bold leading-none mt-0.5" style={{ color: gradeColor }}>{letterGrade}</span>
                      )}
                    </div>
                  )}
                </div>
            ...
            {!isNG && !isCI && !isPct && (
              <div>
                <div className="flex items-center gap-2">
                  <input
                    type="number" min={0} max={panel.maxPoints} step={0.5}
                    value={pointsInput}
            ...
            <div className="px-5 py-4 border-b border-gray-100">
              ...
              {panel.grade.fileUrl && (
                <button onClick={() => panel.grade.submissionId && onOpenSpeedgrader(panel.staffId, panel.assignmentId, panel.grade.submissionId!)}
                  ...
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center shrink-0" style={{ background: MAROON_LIGHT }}>
...
function FormResponsePanel({
  ...
  const pctValue = gradeInput && panel.maxPoints > 0
    ? Math.round((parseFloat(gradeInput) / panel.maxPoints) * 100) : null;
    ...
    <div data-cell-editor className="flex flex-col w-full">
      <div className="flex items-stretch w-full border-2 rounded" style={{ borderColor: "#2563eb" }}>
        <input
          ref={inputRef}
          autoFocus
          type="number" min={0} max={col.points} step={0.5}
          defaultValue={score ?? ""}
          className="flex-1 h-9 text-sm font-semibold text-center bg-white outline-none w-0"
          onClick={e => e.stopPropagation()}
          onKeyDown={e => { if (e.key === "Escape") onDismiss(); }}
...
export default function CourseGradesPage({ courseId }: { courseId: string }) {
  ...
  const activeGroupFilters = activeFilters
    .filter(f => f.type === "assignmentGroup")
    .map(f => f.value);
        ...
        <div className="flex items-center gap-3">
          ...
          {totalPending > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-black text-white bg-white/20">
              <Clock size={9} />{totalPending} missing
            </span>
          )}
        </div>
            ...
            <thead>
              <tr>
                ...
                {filteredColumns.map(col => {
                  ...
                  const subLabel = isNG ? "not graded"
                    : isPct ? "percentage"
                    : isCI ? "complete or incomplete"
                    : "points";
                ...
                <tr key={staff.id} className="group hover:bg-blue-50/30 transition-colors border-b border-gray-100">
                    ...
                    return (
                      <td key={col.id} data-grade-cell
                        ...
                        {isActive ? (
                          ...
                          <div
                            className="flex items-center justify-center w-full h-9 cursor-pointer relative"
                  ...
                  {visibleGroups.map(group => {
                    const groupCols = allColumns.filter(c =>
                      (c.assignmentGroup || "Ungrouped") === group &&
                      !c.doNotCount &&
                      c.displayGradeAs !== "Not Graded"
                    );
                    const groupEarned = groupCols.reduce((sum, col) => {
                      if (col.type === "form") {
                        return sum + (staff.formGrades?.find(g => g.formId === col.id)?.score ?? 0);
                      }
