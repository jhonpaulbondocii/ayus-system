...
function RepoCard({ row, selected, onClick }: { row: Row; selected: boolean; onClick: () => void }) {
  return (
    <div onClick={onClick} style={{ background: selected ? "#fdf2f2" : "#fff", border: `1px solid ${selected ? "rgba(123,17,19,0.25)" : "#e5e7eb"}`, borderLeft: `3px solid ${selected ? MAROON : "transparent"}`, borderRadius: 12, padding: "12px 14px", cursor: "pointer", transition: "all 0.15s", boxShadow: selected ? "0 2px 8px rgba(123,17,19,0.08)" : "0 1px 3px rgba(0,0,0,0.04)" }}>
...
function RepoRow({ row, selected, onClick }: { row: Row; selected: boolean; onClick: () => void }) {
  return (
    <div onClick={onClick}
      style={{ display: "flex", alignItems: "center", gap: 16, padding: "13px 20px", borderBottom: "1px solid #f3f4f6", cursor: "pointer", transition: "background 0.1s", background: selected ? "#fdf2f2" : "transparent", borderLeft: `3px solid ${selected ? MAROON : "transparent"}` }}
      onMouseEnter={e => { if (!selected) e.currentTarget.style.background = "#fafafa"; }}
      onMouseLeave={e => { if (!selected) e.currentTarget.style.background = "transparent"; }}
    >
      <div style={{ width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, background: row.kind === "assignment" ? "#fef2f2" : "#eff6ff" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, color: "#9ca3af" }}>
          {row.kind === "assignment" && <><span>{row.fileCount} file{row.fileCount !== 1 ? "s" : ""}</span><span style={{ color: "#e5e7eb" }}>·</span></>}
...
// ─── Student Section (submissions tab) ───────────────────────────────────────
function StudentSection({ user, files, points, onPreview }: {
  user: { id: string; name: string | null; email: string; image: string | null };
  files: RepoFile[];
  points: number;
  onPreview: (f: RepoFile) => void;
}) {
...
function RepositoryDrawer({ row, courseId, onClose }: { row: Row; courseId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoadingForm(true);
      try {
        const res  = await fetch(`/api/admin/courses/${courseId}/forms/${row.id}/submissions`, { signal: controller.signal });
        const json = await res.json();
        setFormSubs(json.submissions ?? []);
      } catch {
        if (!controller.signal.aborted) setFormSubs([]);
      } finally {
        if (!controller.signal.aborted) setLoadingForm(false);
      }
    };
    load();
  ...
  const goFullView = () => {
    if (row.kind === "assignment") {
      router.push(row.repoId
        ...
        : `/admin/courses/${courseId}/assignments/${row.assignmentId}`);
    } else {
    ...
    <>
      ...
      <div style={drawerStyle}>
              ...
              {row.kind === "assignment" && editingName ? (
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <button onClick={saveName} disabled={savingName}
                    style={{ fontSize: 11, fontWeight: 700, padding: "4px 10px", borderRadius: 6, background: "rgba(255,255,255,0.2)", color: "#fff", border: "none", cursor: "pointer" }}>
                    {savingName ? "…" : "Save"}
                  </button>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  {row.kind === "assignment" && row.repoId && (
                    <button onClick={() => { setNameInput(row.name); setEditingName(true); }}
                      style={{ color: "rgba(255,255,255,0.45)", background: "none", border: "none", cursor: "pointer", display: "flex", flexShrink: 0 }}>
                      <Pencil size={12}/>
                    </button>
                  )}
                </div>
        ...
        <div style={{ display: "flex", alignItems: "center", borderBottom: "1px solid #f3f4f6", background: "#fdf8f8", flexShrink: 0, overflowX: "auto" }}>
          {row.kind === "assignment" ? (
            <>
              <div style={{ padding: isMobile ? "8px 14px" : "10px 16px", borderRight: "1px solid #f3f4f6" }}>
        ...
        {row.kind === "assignment" && (
          <div style={{ padding: "8px 16px", background: "#fafafa", borderBottom: "1px solid #f3f4f6", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "#6b7280" }}>
              <span style={{ fontWeight: 700, color: "#1f2937" }}>{row.name}</span>
              {row.points > 0 && <><span style={{ color: "#e5e7eb" }}>·</span><span style={{ fontWeight: 600, color: MAROON }}>{row.points} pts</span></>}
            </div>
...
export default function AdminCourseRepositoriesPage({ courseId }: { courseId: string }) {
  ...
  return (
    <>
          ...
          <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 14, overflow: "hidden", display: "flex", flexDirection: "column", boxShadow: "0 1px 4px rgba(0,0,0,0.04)", flex: 1, minHeight: 300 }}>
            ...
            {loading ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: 12, overflowY: "auto" }}>
                {filtered.map(row => (
                  <RepoCard key={`${row.kind}-${row.id}`} row={row}
              ...
              <div style={{ display: "flex", flexDirection: "column", flex: 1, overflowY: "auto" }}>
                {filtered.map(row => (
                  <RepoRow key={`${row.kind}-${row.id}`} row={row}
                    selected={drawerRow?.id === row.id && drawerRow?.kind === row.kind}
                    onClick={() => setDrawerRow(prev => prev?.id === row.id && prev?.kind === row.kind ? null : row)}
                  />
                ))}
      ...
      {filterOpen && (
        <MobileFilterSheet tab={tab} setTab={setTab} sort={sort} setSort={setSort}
          onClose={() => setFilterOpen(false)} tabItems={tabItems}/>
      )}

      {drawerRow && (
        <RepositoryDrawer row={drawerRow} courseId={courseId} onClose={() => setDrawerRow(null)}/>
      )}
    </>
  );
}