  ...
  function parseRoles(raw: string | null | undefined): CourseRole[] {
    if (!raw) return ["Staff"];
    const r = raw.trim() as CourseRole;
    return ALL_ROLES.includes(r) ? [r] : ["Staff"];
  }
  ...
  /* Desktop table */
  .cpp-table-wrap { width:100%; overflow:visible; position:relative; z-index:1; }
  .cpp-table { width:100%; border-collapse:collapse; font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; min-width:520px; overflow:visible; }
  .cpp-table tbody { position:relative; }
  .cpp-table td:last-child { overflow:visible; position:relative; }
  ...
  /* Menus */
  .cpp-menu       { position:absolute; background:#fff; border:1px solid #f0e4e4; border-radius:12px; box-shadow:0 4px 20px rgba(123,17,19,.1); z-index:10000; min-width:180px; padding:4px 0; overflow:hidden; }
  .cpp-menu-fixed { position:fixed;    background:#fff; border:1px solid #f0e4e4; border-radius:12px; box-shadow:0 4px 20px rgba(123,17,19,.1);  z-index:100000; min-width:176px; padding:4px 0; overflow:hidden; }
  ...
  const RolePills = ({ raw }: { raw: string }) => {
    const role = parseRoles(raw)[0] ?? "Staff";
    return (
      <span className="cpp-badge" style={{ background: ROLE_BADGE[role].bg, color: ROLE_BADGE[role].color }}>
        {role}
      </span>
    );
  };
  ...
  export default function CoursePeoplePage() {
    ...
    const openPersonMenu = (personId: string, button: HTMLButtonElement | null) => {
      if (!button) return;
      const rect = button.getBoundingClientRect();
      const menuWidth = 180;
      const menuHeight = 170;
      const nextLeft = Math.min(rect.right - menuWidth + 6, window.innerWidth - menuWidth - 8);
      const nextTop = Math.min(rect.bottom + 8, window.innerHeight - menuHeight - 12);

      setMenuOpenId((current) => (current === personId ? null : personId));
      setMenuPosition({
    ...
    useEffect(() => {
      const handler = (e: MouseEvent) => {
        ...
        setMenuOpenId(null); setMenuPosition(null); setGsMenuOpen(null); setGroupMenuOpen(null); setMemberMenuOpen(null); setAddToGroupMenu(null);
      };
    ...
    const closeAddModal = () => { setAddModal(false); setChips([]); setSearchQ(""); setSuggestions([]); setAddError(""); setAddRoles(["Staff"]); setSelectedIds(new Set()); setBrowseSearch(""); setBulkError(""); setAddModalTab("search"); };
    ...
    const removeUser = async (userId:string) => {
      ...
      startTransition(()=>setPeople(p=>p.filter(x=>x.id!==userId))); setMenuOpenId(null); setMenuPosition(null);
    };

    /* ── Edit Roles ─────────────────────────────────────────────────────────── */
    const openEditRoles = (p:Person) => { setEditRolesTarget(p); setEditRolesChecked(parseRoles(p.role)); setMenuOpenId(null); setMenuPosition(null); };
    ...
    const openChangeOffice = async (p:Person) => {
      setChangeOfficeTarget(p); setSelectedNewCourse(""); setMenuOpenId(null); setMenuPosition(null);
    ...
    /* ── Member operations ──────────────────────────────────────────────────── */
    const toggleGroup = (id: string) => setExpandedGroups((p) => {
      const s = new Set(p);
    ...
    ALL_ROLES.forEach(r=>{ roleCounts[r]=people.filter(p=>parseRoles(p.role)[0]===r).length; });
    ...
    const toggleUser = (id: string) => setSelectedIds((p) => {
      const s = new Set(p);
      if (s.has(id)) s.delete(id);
      else s.add(id);
      return s;
    ...
    <div style={{position:"relative",display:"inline-block", zIndex: menuOpenId === p.id ? 10001 : 1}}>
      <button
        ...
        onClick={e=>{e.stopPropagation(); openPersonMenu(p.id, e.currentTarget);}}
      >
        <MoreVertical size={15}/>
      </button>
      {menuOpenId===p.id && menuPosition && (
        <div className="cpp-menu-fixed" style={{ top: menuPosition.top, left: menuPosition.left }} onClick={e=>e.stopPropagation()}>
          <button className="cpp-menu-item" onClick={()=>{router.push(`/admin/courses/${courseId}/people/${p.id}`);setMenuOpenId(null); setMenuPosition(null);}}>View Profile</button>
              ...
              <div className="cpp-modal" style={{maxWidth:420}}>
                ...
                <div className="cpp-modal-body">
                  ...
                  <div className="cpp-role-check-row">
                    {ALL_ROLES.map(r=>{
                      const selected = editRolesChecked[0] === r;
                      const desc = r === "Head"
                        ? "Full access — create, edit, and delete everything."
                        : r === "Staff"
                        ? "Can create and edit own assignments/forms. Cannot delete."
                        : "View and submit only. No create/edit/delete.";
                      return(
                        <label key={r} className={`cpp-role-check-item${selected?" selected":""}`}>
                          <input type="radio" name="editRole" checked={selected}
                            onChange={()=>setEditRolesChecked([r])}
                            style={{accentColor:"#7b1113",marginTop:2,width:16,height:16,flexShrink:0,cursor:"pointer"}}/>
                          <div>
                            <span style={{fontWeight:700,fontSize:13,color:selected?ROLE_BADGE[r].color:"#2d3b45"}}>{r}</span>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:16,marginBottom:12}}>
                      <div>
                        ...
                        <div className="cpp-role-check-row" style={{flexDirection:"row",gap:8}}>
                          {ALL_ROLES.map(r=>{
                            const selected = addRoles[0] === r;
                            return(
                              <label key={r} className={`cpp-role-check-item${selected?" selected":""}`} style={{flex:1,padding:"10px 12px"}}>
                                <input type="radio" name="addRole" checked={selected}
                                  onChange={()=>setAddRoles([r])}
                                  style={{accentColor:"#7b1113",width:15,height:15,flexShrink:0,cursor:"pointer"}}/>
                                <span style={{fontWeight:700,fontSize:13,color:selected?ROLE_BADGE[r].color:"#2d3b45"}}>{r}</span>
                              </label>
                            );
                          })}
                    ...
                    <div style={{padding:"16px 20px 12px",flexShrink:0}}>
                      <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
                        ...
                        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                          {ALL_ROLES.map(r=>{
                            const selected = browseRoles[0] === r;
                            return(
                              <label key={r} className={`cpp-role-check-item${selected?" selected":""}`} style={{padding:"6px 12px",flexDirection:"row",alignItems:"center",gap:6,cursor:"pointer"}}>
                                <input type="radio" name="browseRole" checked={selected}
                                  onChange={()=>setBrowseRoles([r])}
                                  style={{accentColor:"#7b1113",width:14,height:14,cursor:"pointer"}}/>
                                <span style={{fontWeight:700,fontSize:12,color:selected?ROLE_BADGE[r].color:"#2d3b45"}}>{r}</span>
                              </label>
                            );
                          })}
                        </div>
