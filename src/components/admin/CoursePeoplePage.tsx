...
/* ── Table ── */
.cpp-table-wrap { width:100%; overflow-x:auto; -webkit-overflow-scrolling:touch; }
...
/* ── Mobile card list ── */
.cpp-people-card { display:none; flex-direction:column; gap:10px; }
.cpp-person-card { display:flex; align-items:center; gap:10px; background:#fff; border:1px solid #f0e4e4; border-radius:12px; padding:12px 14px; position:relative; }
.cpp-person-card:nth-child(even) { background:#fdf8f8; }
.cpp-person-card-info { flex:1; min-width:0; }
.cpp-person-card-name  { font-size:13px; font-weight:700; color:#7b1113; margin-bottom:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; cursor:pointer; }
.cpp-person-card-name:hover { text-decoration:underline; }
.cpp-person-card-email { font-size:12px; color:#9ca3af; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.cpp-person-card-role  { font-size:11px; font-weight:700; color:#7b1113; margin-top:4px; }
...
.cpp-menu-fixed { position:fixed;    background:#fff; border:1px solid #f0e4e4; border-radius:12px; box-shadow:0 4px 20px rgba(123,17,19,.1);  z-index:9999; min-width:176px; padding:4px 0; overflow:hidden; }
...
.cpp-member-card   { display:flex; align-items:center; gap:8px; border:1px solid #f0e4e4; background:#fff; border-radius:8px; padding:7px 10px; transition:border-color .15s; }
...
@media (max-width: 768px) {
  ...
  .cpp-tab    { padding:8px 12px; font-size:12px; }
  .cpp-tabbar-actions { display:flex; justify-content:flex-end; padding:8px 0 4px; }

  .cpp-toolbar { flex-direction:column; align-items:stretch; }
  .cpp-toolbar-left { flex-direction:column; align-items:stretch; }
  .cpp-toolbar-left > div { width:100%; }
  ...
  .cpp-people-card { display:flex; }

  .cpp-groups-layout { flex-direction:column; gap:16px; }
  .cpp-unassigned-col { width:100%; }
  .cpp-group-members  { padding:10px 16px; }
  .cpp-group-members > div { grid-template-columns:1fr !important; }
  .cpp-group-row span[style*="marginLeft: 12"] { display:none; }
  ...
  .cpp-subtabs { padding:0 16px; }
  .cpp-modal-title  { font-size:15px; }

  .cpp-modal-grid-2col { display:flex !important; flex-direction:column !important; gap:8px !important; }

  .cpp-browse-th.hide-mobile  { display:none; }
  .cpp-browse-td.hide-mobile  { display:none; }

  .cpp-side-panel { width:100%; border-left:none; border-top:1px solid #f0e4e4; }

  .cpp-gs-actions { justify-content:flex-start; }
}

@media (max-width: 480px) {
  .cpp-root { padding:8px 10px 80px; }
  .cpp-tab  { padding:7px 10px; font-size:11px; }
  .cpp-person-card { padding:10px 12px; gap:8px; }
  .cpp-group-row   { gap:6px; padding:10px 8px; }
}
`;
...
/* ─────────────────────────────────────────────────────────────────────────────
   HELPERS
───────────────────────────────────────────────────────────────────────────── */
function normalizeCourseRole(role: string): "Staff" | "Head" {
...
const SpinnerInput = ({ value, onChange, placeholder, width = 160 }: {
  ...
  <div className="cpp-spin-wrap" style={{ width }}>
    <input type="number" min={0} value={value || ""} onChange={e => onChange(parseInt(e.target.value) || 0)}
      placeholder={placeholder ?? "No limit"} className="cpp-spin-input" />
...
export default function CoursePeoplePage() {
  ...
  // refs for person-row trigger buttons (keyed by person id)
...
document.addEventListener("click", handler);
  ...
  const fetchPeople = () => {
    ...
    fetch(`/api/admin/courses/${courseId}/people`)
      ...
      .then(d => startTransition(() => {
        setPeople((d.people ?? []).map((p: Person) => ({ ...p, role: normalizeCourseRole(p.role) })));
        setLoading(false);
      }))
  ...
  useEffect(() => {
    ...
    if (tab === "groups" && gs) {
      ...
      if (found) { setActiveTab(found.id); router.replace(`/admin/courses/${courseId}/people`, { scroll:false }); }
      else setActiveTab("groups");
    } else if (tab === "groups") {
      if (groupSets.length > 0) { setActiveTab(groupSets[0].id); router.replace(`/admin/courses/${courseId}/people`, { scroll:false }); }
      else setActiveTab("groups");
    }
  ...
  const loadAllUsers = async () => {
    ...
    try {
      ...
      setAllUsers((d.users ?? []).filter((u: UserSuggestion & { role?:string }) =>
        !enrolled.has(u.email.toLowerCase()) && u.role !== "ADMIN"
      ));
  ...
  const loadSuggestions = async (q: string) => {
    ...
    try {
      ...
      setSuggestions((d.users ?? []).filter((u: UserSuggestion) =>
        !enrolled.has(u.email.toLowerCase()) && !chipped.has(u.email.toLowerCase())
      ).slice(0, 8));
  ...
  const addChipFromSugg = (u: UserSuggestion) => {
    setChips(p => [...p, { id:crypto.randomUUID(), userId:u.id, name:u.name, email:u.email, image:u.image, status:"valid" }]);
    setSearchQ(""); setSuggestions([]); setShowSugg(false);
    searchRef.current?.focus();
  };

  const commitQ = () => {
    const v = searchQ.trim(); if (!v) return;
    setChips(p => [...p, { id:crypto.randomUUID(), name:v, email:v, image:null, status:"pending" }]);
    setSearchQ(""); setSuggestions([]); setShowSugg(false);
  };
  ...
  const handleKD = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if ((e.key === "Enter" || e.key === ",") && searchQ.trim()) { e.preventDefault(); commitQ(); }
    if (e.key === "Backspace" && searchQ === "" && chips.length > 0) setChips(p => p.slice(0, -1));
  };

  const handlePaste = (e: React.ClipboardEvent<HTMLInputElement>) => {
    const emails = e.clipboardData.getData("text").split(/[\n,;]+/).map(s => s.trim()).filter(Boolean);
    if (emails.length > 1) {
      e.preventDefault();
      emails.forEach(email => setChips(p => [...p, { id:crypto.randomUUID(), name:email, email, image:null, status:"pending" }]));
      setSearchQ("");
    }
  };
  ...
  const handleAddPeople = async () => {
    ...
    for (const chip of chips) {
      try {
        ...
        bad.push(chip.email);
        setChips(p => p.map(c => c.id === chip.id ? { ...c, status:"invalid" } : c));
      }
    }
  ...
  const handleBulkAdd = async () => {
    ...
    await new Promise<void>(resolve => {
      fetch(`/api/admin/courses/${courseId}/people`)
        ...
        .then(d => {
          const up = d.people ?? [];
          startTransition(() => { setPeople(up.map((p: Person) => ({ ...p, role:normalizeCourseRole(p.role) }))); setLoading(false); });
  ...
  const closeAddModal = () => {
    setAddModal(false); setChips([]); setSearchQ(""); setSuggestions([]);
    setAddError(""); setAddRole("Staff"); setSectionOnly(false);
    setSelectedIds(new Set()); setBrowseSearch(""); setBulkError(""); setAddModalTab("search");
  };
  ...
  const updatePersonRole = async (userId: string, nextRole: "Staff"|"Head") => {
    ...
    await fetch(`/api/admin/courses/${courseId}/people`, {
      method:"POST", headers:{ "Content-Type":"application/json" },
      body: JSON.stringify({ email:person.email, role:nextRole }),
    });
    setPeople(prev => prev.map(p => p.id === userId ? { ...p, role:nextRole } : p));
    setMenuOpenId(null);
  };
  ...
  const deleteGs = async (id: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets?groupSetId=${id}`, { method:"DELETE" });
  ...
  const handleCloneGs = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const d = await res.json(); const newId = d.groupSet?.id;
        ...
        await fetchGroupSetsAsync(); if (newId) setActiveTab(newId);
      }
      setCloneModal(null);
  ...
  const deleteGroup = async (gsId: string, gId: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}`, { method:"DELETE" });
  ...
  const removeMember = async (gsId: string, gId: string, userId: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}/members/${userId}`, { method:"DELETE" });
  ...
  const removeLeader = async (gsId: string, gId: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}/leader`, { method:"DELETE" });
    fetchGroupSets(); setMemberMenuOpen(null);
  };
  ...
  const handleMoveStudent = async () => {
    ...
    try {
      await fetch(`/api/admin/courses/${courseId}/groupsets/${movePanel.groupSetId}/groups/${movePanel.fromGroupId}/members/${movePanel.userId}`, { method:"DELETE" });
      await fetch(`/api/admin/courses/${courseId}/groupsets/${movePanel.groupSetId}/groups/${moveToGroupId}/members`, {
        method:"POST", headers:{ "Content-Type":"application/json" },
        body: JSON.stringify({ userId:movePanel.userId, placement:movePlacement, relativeUserId:moveRelativeUserId || undefined }),
      });
  ...
  const roleCounts = ALL_DISPLAY_ROLES.reduce<Record<string,number>>((a, r) => {
    a[r] = people.filter(p => p.role.toLowerCase() === r.toLowerCase()).length; return a;
  }, {});
      ...
      <div className="cpp-root">
        ...
        <div className="cpp-tabbar">
          <div className="cpp-tabs">
            {tabs.map(t => (
              <button key={t.key} className={`cpp-tab${activeTab === t.key ? " active" : ""}`} onClick={() => setActiveTab(t.key)}>
                {t.label}
              </button>
            ))}
          </div>
          <div className="cpp-tabbar-actions" style={{ paddingBottom: 6 }}>
            <button className="cpp-btn-primary" onClick={() => { resetGsModal(); setGroupSetModal(true); }}>
              + Group Set
            </button>
          </div>
        </div>
            ...
            <div className="cpp-toolbar">
              <div className="cpp-toolbar-left">
                <div style={{ position:"relative" }}>
                  ...
                  <input className="cpp-input" value={search} onChange={e => setSearch(e.target.value)}
                ...
                <div style={{ position:"relative" }}>
                  <select className="cpp-select" value={roleFilter} onChange={e => setRoleFilter(e.target.value)} style={{ minWidth:160 }}>
                    ...
                    {ALL_DISPLAY_ROLES.map(r => roleCounts[r] > 0 ? <option key={r} value={r}>{r} ({roleCounts[r]})</option> : null)}
                  </select>
                  <DropArrow />
                </div>
              </div>
              <button className="cpp-btn-primary" onClick={() => { closeAddModal(); setAddModal(true); }}>
                + People
              </button>
            </div>
              ...
              <>
                ...
                <div className="cpp-table-wrap">
                  <table className="cpp-table">
                    ...
                    <tbody>
                      {filtered.length === 0 ? (
                        <tr><td colSpan={6} style={{ padding:"48px 0", textAlign:"center", fontSize:13, color:"#9ca3af" }}>No people enrolled yet.</td></tr>
                      ) : filtered.map((p, i) => (
                        <tr key={p.id} className={i % 2 === 0 ? "even" : "odd"}>
                          ...
                          <td>
                            ...
                            <button
                              ref={el => {
                                if (el) personBtnRefs.current.set(p.id, el);
                ...
                <div className="cpp-people-card">
                  {filtered.length === 0 ? (
                    ...
                    <div key={p.id} className="cpp-person-card">
                      <Avatar name={p.name} image={p.image} size={38} />
                      <div className="cpp-person-card-info">
                        <div className="cpp-person-card-name" onClick={() => router.push(`/admin/courses/${courseId}/people/${p.id}`)}>
                          {p.name}
                      ...
                      <div style={{ flexShrink:0 }}>
                        ...
                        <button
                          ref={el => {
                            if (el) personBtnRefs.current.set(`mobile-${p.id}`, el);
                        ...
                        {menuOpenId === p.id && (
                          <div
                            ...
                            ref={el => {
                              if (!el) return;
                              const btn = personBtnRefs.current.get(`mobile-${p.id}`);
                              if (!btn) return;
                              const r = btn.getBoundingClientRect();
                              el.style.top  = `${r.bottom + 4}px`;
                              el.style.left = `${Math.max(4, r.right - el.offsetWidth)}px`;
        ...
        {activeGs && (
          <>
            <div className="cpp-gs-actions">
              <button className="cpp-btn-secondary" onClick={() => { setNewGroupName(""); setNewGroupLimit(0); setAddGroupModal(activeGs.id); }}>
                + Group
              </button>
              <div style={{ position:"relative" }}>
            ...
            <div className="cpp-groups-layout">
              ...
              <div className="cpp-unassigned-col">
                ...
                {unassigned.length === 0 ? (
                  <div className="cpp-box-dashed cpp-empty-box">No unassigned members.</div>
                ) : (
                          ...
                          {addToGroupMenu === p.id && activeGs.groups.length > 0 && (
                            <div className="cpp-menu" style={{ right:0, top:28 }} onClick={e => e.stopPropagation()}>
                              ...
                              {activeGs.groups.map(g => (
                                <button key={g.id} className="cpp-menu-item" style={{ color:"#7b1113" }} onClick={() => void addStudentToGroup(activeGs.id, g.id, p.id)}>{g.name}</button>
                              ))}
                            </div>
                          )}
                        </div>
                      ...
                      return (
                        <div key={g.id}>
                          <div className="cpp-group-row">
                            ...
                            {leader && (
                              <span style={{ display:"flex", alignItems:"center", gap:4, fontSize:12, color:"#6b7780", flexShrink:0 }}>
                                  ...
                                  {g.members.map(m => {
                                    ...
                                    return (
                                      <div key={m.user.id} className="cpp-member-card">
                                        ...
                                        <div style={{ position:"relative" }}>
                                          <button className="cpp-btn-icon" style={{ padding:3 }}
                                            onClick={e => { e.stopPropagation(); setMemberMenuOpen(memberMenuOpen === mk ? null : mk); }}>
                                            <MoreVertical size={14} />
                                          </button>
                                          {memberMenuOpen === mk && (
                                            <div className="cpp-menu-fixed"
                                              onClick={e => e.stopPropagation()}
                                              ref={el => {
                                                if (el) {
                                                  const btn = el.previousElementSibling as HTMLElement;
                                                  if (btn) { const r = btn.getBoundingClientRect(); el.style.top = `${r.bottom+4}px`; el.style.left = `${Math.max(4, r.right-176)}px`; }
                                                }
                                              }}>
                                              <button className="cpp-menu-item" onClick={() => void removeMember(activeGs.id, g.id, m.user.id)}>Remove</button>
                                              {m.isLeader
                                                ? <button className="cpp-menu-item" onClick={() => void removeLeader(activeGs.id, g.id)}>Remove as Leader</button>
                                                : <button className="cpp-menu-item" onClick={() => void setLeader(activeGs.id, g.id, m.user.id)}>Set as Leader</button>}
                                              <button className="cpp-menu-item" onClick={() => {
                                                ...
                                                setMoveToGroupId(first?.id ?? ""); setMovePlacement("At the Top"); setMoveRelativeUserId(""); setMemberMenuOpen(null);
                                              }}>Move To...</button>
                                            </div>
                                          )}
                                        </div>
                ...
                <div className="cpp-modal-body">
                  ...
                  <div style={{ marginBottom:16 }}>
                    ...
                    <div className="cpp-chip-box" onClick={() => searchRef.current?.focus()}>
                      {chips.map(chip => (
                        <span key={chip.id} style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 6px 3px 8px", borderRadius:6, fontSize:12,
                          background: chip.status === "valid" ? "#f0fdf4" : "#fef2f2",
                          border: `1px solid ${chip.status === "valid" ? "#86efac" : "#f0c0c0"}`,
                          color: chip.status === "valid" ? "#15803d" : "#7b1113" }}>
                          ...
                          <button onMouseDown={e => { e.preventDefault(); removeChip(chip.id); }} style={{ background:"none", border:"none", cursor:"pointer", padding:0, display:"flex", color:"inherit", opacity:0.7 }}>
                            <X size={11} />
                          </button>
                        </span>
                      ))}
                      <input ref={searchRef} className="cpp-chip-input" value={searchQ}
                        onChange={e => setSearchQ(e.target.value)} onKeyDown={handleKD} onPaste={handlePaste}
                    ...
                    {showSugg && (suggestions.length > 0 || suggLoading) && (
                      <div className="cpp-sugg">
                        {suggLoading && !suggestions.length && (
                          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", fontSize:13, color:"#9ca3af" }}>
                            <Loader2 size={14} className="cpp-spin" />Searching…
                          </div>
                        )}
                        {suggestions.map(u => (
                          <button key={u.id} className="cpp-sugg-item" onMouseDown={e => { e.preventDefault(); addChipFromSugg(u); }}>
                            ...
                            <div>
                              <p style={{ fontSize:13, fontWeight:700, color:"#2d3b45", margin:0 }}>{u.name}</p>
                  ...
                  <div className="cpp-modal-grid-2col" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:12 }}>
                    <div>
                      <label style={{ display:"block", fontSize:13, fontWeight:700, color:"#2d3b45", marginBottom:6 }}>Role</label>
                      <div style={{ position:"relative" }}>
                        <select className="cpp-select" value={addRole} onChange={e => setAddRole(normalizeCourseRole(e.target.value))} style={{ width:"100%" }}>
                          {ENROLLMENT_ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                        </select>
                        <DropArrow />
                      </div>
                    </div>
              ...
              {addModalTab === "browse" && (
                <div style={{ display:"flex", flexDirection:"column", flex:1, overflow:"hidden" }}>
                  <div style={{ padding:"16px 20px 12px", flexShrink:0 }}>
                    <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
                      <div style={{ position:"relative", flex:1, minWidth:160 }}>
                        ...
                        <input className="cpp-input" value={browseSearch} onChange={e => setBrowseSearch(e.target.value)}
                          placeholder="Search users by name or email…" style={{ paddingLeft:30, width:"100%", boxSizing:"border-box" }} />
                      </div>
                      <div style={{ position:"relative" }}>
                        <select className="cpp-select" value={browseRole} onChange={e => setBrowseRole(normalizeCourseRole(e.target.value))}>
                          {ENROLLMENT_ROLES.map(r => <option key={r} value={r}>{r}</option>)}
                        </select>
                        <DropArrow />
                      </div>
                    ...
                    {selectedIds.size > 0 && (
                      <div className="cpp-banner">
                        <span style={{ fontSize:13, color:"#7b1113", fontWeight:600 }}>
                          {selectedIds.size} user{selectedIds.size !== 1 ? "s" : ""} selected — will be added as <strong>{browseRole}</strong>
                        </span>
                  ...
                  <div style={{ flex:1, overflowY:"auto", borderTop:"1px solid #f0e4e4" }}>
                    {browseLoading ? (
                      <div style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"40px 0", gap:8, color:"#9ca3af", fontSize:13 }}>
                        <Loader2 size={18} className="cpp-spin" />Loading users…
                      </div>
                    ) : browseFiltered.length === 0 ? (
                      <p style={{ fontSize:13, color:"#9ca3af", textAlign:"center", padding:"40px 0" }}>No users found.</p>
                    ) : (
                      <table className="cpp-browse-table">
                        <thead className="cpp-browse-thead">
                          <tr>
                            <th style={{ width:40, padding:"10px 14px" }}>
                              <input type="checkbox" checked={allBrowseSel}
                                ref={el => { if (el) el.indeterminate = someBrowseSel && !allBrowseSel; }}
                                onChange={toggleAll} style={{ width:14, height:14, accentColor:"#7b1113", cursor:"pointer" }} />
                            </th>
                            <th className="cpp-browse-th">Name</th>
                            <th className="cpp-browse-th hide-mobile">Email</th>
                          </tr>
                        </thead>
                        <tbody>
                              ...
                              <td style={{ padding:"10px 12px" }}>
                                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                                  ...
                                  <div>
                                    ...
                                    <p style={{ fontSize:11, color:"#9ca3af", margin:0, display:"none" }} className="browse-email-mobile">{u.email}</p>
                                  </div>
                                </div>
                              </td>
                              <td className="hide-mobile" style={{ padding:"10px 12px", fontSize:13, color:"#6b7780" }}>{u.email}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
        ...
        {groupSetModal && (
          <div className="cpp-overlay" onClick={e => { if (e.target === e.currentTarget) setGroupSetModal(false); }}>
            <div className="cpp-modal" style={{ maxWidth:560, maxHeight:"90vh" }}>
              ...
              <div className="cpp-modal-body">
                <div className="cpp-modal-grid-2col" style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:12, alignItems:"center", marginBottom:20 }}>
                ...
                <div className="cpp-modal-grid-2col" style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:12, alignItems:"flex-start", borderTop:"1px solid #f0e4e4", paddingTop:20, marginBottom:20 }}>
                ...
                <div className="cpp-modal-grid-2col" style={{ display:"grid", gridTemplateColumns:"160px 1fr", gap:12, alignItems:"flex-start", borderTop:"1px solid #f0e4e4", paddingTop:20 }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    ...
                    {[{ val:"first", label:"Set first member to join as group leader" }, { val:"random", label:"Set a random member as group leader" }].map(opt => (
                      <label key={opt.val} style={{ display:"flex", alignItems:"center", gap:8, cursor:autoAssignLeader?"pointer":"default", fontSize:13, color:autoAssignLeader?"#2d3b45":"#9ca3af", paddingLeft:8 }}>
                        <input type="radio" name="leaderType" value={opt.val} checked={leaderType === opt.val} onChange={() => setLeaderType(opt.val)} disabled={!autoAssignLeader} style={accentM} />
                        {opt.label}
                      </label>
                    ))}
                  </div>
              ...
              <div className="cpp-modal-footer">
                ...
                <button className="cpp-btn-primary" onClick={() => void handleCreateGs()} disabled={savingGroupSet || !groupSetName.trim()}>
          ...
          <div className="cpp-overlay" onClick={e => { if (e.target === e.currentTarget) setAddGroupModal(null); }}>
            <div className="cpp-modal" style={{ maxWidth:440 }}>
              ...
              <div className="cpp-modal-footer">
                ...
                <button className="cpp-btn-primary" onClick={() => void handleAddGroup()} disabled={savingGroup || !newGroupName.trim()}>
          ...
          <div className="cpp-overlay" onClick={e => { if (e.target === e.currentTarget) setEditGroupModal(null); }}>
            <div className="cpp-modal" style={{ maxWidth:380 }}>
              ...
              <div className="cpp-modal-footer">
                ...
                <button className="cpp-btn-primary" onClick={() => void handleEditGroup()} disabled={savingEditGroup || !editGroupName.trim()}>
          ...
          <div className="cpp-overlay" onClick={e => { if (e.target === e.currentTarget) setEditGsModal(null); }}>
            <div className="cpp-modal" style={{ maxWidth:560, maxHeight:"90vh" }}>
                ...
                <div style={{ borderTop:"1px solid #f0e4e4", paddingTop:16, display:"flex", flexDirection:"column", gap:10 }}>
                  ...
                  {[{ val:"first", label:"Set first member to join as group leader" }, { val:"random", label:"Set a random member as group leader" }].map(opt => (
                    <label key={opt.val} style={{ display:"flex", alignItems:"center", gap:8, cursor:editGsAutoAssignLeader?"pointer":"default", fontSize:13, color:editGsAutoAssignLeader?"#2d3b45":"#9ca3af", paddingLeft:8 }}>
                      <input type="radio" name="editLeaderType" value={opt.val} checked={editGsLeaderType === opt.val} onChange={() => setEditGsLeaderType(opt.val)} disabled={!editGsAutoAssignLeader} style={accentM} />
                      {opt.label}
                    </label>
                  ))}
                </div>
              ...
              <div className="cpp-modal-footer">
                ...
                <button className="cpp-btn-primary" onClick={() => void handleEditGs()} disabled={savingEditGs || !editGsName.trim()}>
                  {savingEditGs && <Loader2 size={13} className="cpp-spin" />}Save
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="cpp-overlay" onClick={e => { if (e.target === e.currentTarget) setCloneModal(null); }}>
            <div className="cpp-modal" style={{ maxWidth:440 }}>
              ...
              <div className="cpp-modal-footer">
                ...
                <button className="cpp-btn-primary" onClick={() => void handleCloneGs()} disabled={submittingClone || !cloneName.trim()}>
                  {submittingClone && <Loader2 size={13} className="cpp-spin" />}Submit
                </button>
              </div>
            </div>
          </div>
            ...
            <div style={{ position:"fixed", inset:0, zIndex:50, display:"flex", justifyContent:"flex-end" }}>
              ...
              <div className="cpp-side-panel">
                ...
                <div className="cpp-modal-footer">
                  ...
                  <button className="cpp-btn-primary" onClick={() => void handleMoveStudent()} disabled={movingStudent || !moveToGroupId}>
                    {movingStudent && <Loader2 size={13} className="cpp-spin" />}Move
                  </button>
                </div>
              </div>
            </div>
