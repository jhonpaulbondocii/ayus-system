...
const ENROLLMENT_ROLES = ["Staff", "Head"] as const;
const ALL_DISPLAY_ROLES = ["Staff", "Head"] as const;
...
.cpp-toolbar { display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; }
.cpp-toolbar-left { display:flex; align-items:center; gap:8px; }
.cpp-input {
  font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:13px; color:#2d3b45;
  border:1px solid #e5e7eb; border-radius:8px; padding:6px 10px; outline:none; background:#fff; transition:all .15s;
}
.cpp-input:focus { border-color:#7b1113; box-shadow:0 0 0 3px rgba(123,17,19,.08); }
.cpp-select {
  font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:13px; color:#2d3b45;
  border:1px solid #e5e7eb; border-radius:8px; padding:6px 28px 6px 10px; outline:none; background:#fff;
...
.cpp-btn-primary {
  font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:12px; font-weight:700;
  background:#7b1113; color:#fff; border:none; border-radius:8px; padding:8px 16px; cursor:pointer;
...
.cpp-btn-secondary {
  font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:12px; font-weight:700;
  background:#fff; color:#374151; border:1px solid #e5e7eb; border-radius:8px; padding:7px 14px; cursor:pointer;
  display:inline-flex; align-items:center; gap:6px; transition:all .15s;
}
.cpp-btn-secondary:hover { border-color:#7b1113; color:#7b1113; }
.cpp-btn-icon {
  background:none; border:none; cursor:pointer; padding:4px; border-radius:6px; color:#9ca3af;
  display:flex; align-items:center; transition:all .15s;
}
.cpp-btn-icon:hover { background:#fef2f2; color:#7b1113; }
.cpp-table { width:100%; border-collapse:collapse; font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; }
...
.cpp-table td { padding:10px 10px; font-size:13px; color:#6b7780; }
...
.cpp-menu {
  position:absolute; background:#fff; border:1px solid #f0e4e4;
  border-radius:12px; box-shadow:0 4px 20px rgba(123,17,19,.08); z-index:50; min-width:180px; padding:4px 0; overflow:hidden;
}
.cpp-menu-fixed {
  position:fixed; background:#fff; border:1px solid #f0e4e4;
  border-radius:12px; box-shadow:0 4px 20px rgba(123,17,19,.1); z-index:9999; min-width:176px; padding:4px 0; overflow:hidden;
}
.cpp-menu-item {
  display:block; width:100%; text-align:left;
  font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:12px; font-weight:500; color:#374151;
  padding:8px 14px; background:none; border:none; cursor:pointer; transition:background .1s;
}
...
.cpp-overlay { position:fixed; inset:0; z-index:50; display:flex; align-items:center; justify-content:center; background:rgba(0,0,0,.35); backdrop-filter:blur(2px); }
.cpp-modal {
  background:#fff; border-radius:16px; box-shadow:0 8px 40px rgba(0,0,0,.18); border:1px solid #f0e4e4;
  display:flex; flex-direction:column; font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif;
}
.cpp-modal-header {
  display:flex; align-items:center; justify-content:space-between;
  padding:18px 22px 16px; border-bottom:1px solid #f0e4e4; flex-shrink:0;
}
...
.cpp-modal-footer {
  ...
  padding:14px 22px; border-top:1px solid #f0e4e4;
  background:#fdf8f8; border-radius:0 0 16px 16px; flex-shrink:0;
}
.cpp-modal-body { padding:20px 24px; overflow-y:auto; flex:1; }
.cpp-subtabs { display:flex; border-bottom:1px solid #f0e4e4; padding:0 22px; flex-shrink:0; }
.cpp-subtab { font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:12px; font-weight:600; color:#6b7780; background:none; border:none; border-bottom:2px solid transparent; padding:10px 14px; cursor:pointer; margin-bottom:-1px; transition:color .15s; }
.cpp-subtab:hover  { color:#7b1113; }
.cpp-subtab.active { color:#7b1113; border-bottom-color:#7b1113; }
.cpp-chip-box {
  min-height:44px; border:1px solid #e5e7eb; border-radius:8px; padding:6px 8px;
  display:flex; flex-wrap:wrap; gap:6px; cursor:text; transition:all .15s;
}
.cpp-chip-box:focus-within { border-color:#7b1113; box-shadow:0 0 0 3px rgba(123,17,19,.08); }
.cpp-chip-input { flex:1; min-width:140px; height:28px; font-family:'Plus Jakarta Sans','Helvetica Neue',Arial,sans-serif; font-size:13px; border:none; outline:none; background:transparent; color:#2d3b45; }
...
.cpp-group-members { background:#fdf8f8; border-top:1px solid #f0e4e4; padding:10px 32px; }
.cpp-member-card {
  display:flex; align-items:center; gap:8px;
  border:1px solid #f0e4e4; background:#fff; border-radius:8px; padding:7px 10px; transition:border-color .15s;
}
.cpp-member-card:hover { border-color:#7b1113; }
.cpp-banner {
  display:flex; align-items:center; justify-content:space-between;
  background:#fef2f2; border:1px solid #f0c0c0; border-radius:8px; padding:8px 12px; margin-top:10px;
}
@keyframes cpp-spin { to { transform: rotate(360deg); } }
.cpp-spin { animation: cpp-spin 1s linear infinite; }
.cpp-plus-btn {
  width:24px; height:24px; display:flex; align-items:center; justify-content:center;
  border:1px solid #e5e7eb; border-radius:6px; background:#fff; cursor:pointer; font-size:16px; color:#7b1113; font-weight:700; transition:all .15s;
}
...
function normalizeCourseRole(role: string) {
...
/* ─────────────────────────────────────────────────────────────────────────────
   HELPERS
───────────────────────────────────────────────────────────────────────────── */
function roleBadge(_role: string): React.CSSProperties {
  return { color: "#7b1113", background: "transparent" };
}
...
export default function CoursePeoplePage() {
  ...
  const [addRole, setAddRole] = useState<(typeof ENROLLMENT_ROLES)[number]>("Staff");
  ...
  const [browseRole, setBrowseRole] = useState<(typeof ENROLLMENT_ROLES)[number]>("Staff");
  ...
  const fetchPeople = () => {
    ...
    fetch(`/api/admin/courses/${courseId}/people`)
      ...
      .then((d) =>
        startTransition(() => {
          const normalizedPeople = (d.people ?? []).map((p: Person) => ({
            ...p,
  ...
  const fetchGroupSets = () => {
    ...
    fetch(`/api/admin/courses/${courseId}/groupsets`)
      .then((r) => r.json())
      .then((d) => startTransition(() => setGroupSets(d.groupSets ?? [])))
      .catch(() => {});
  };

  const fetchGroupSetsAsync = (): Promise<void> =>
    new Promise((resolve) => {
      if (!courseId) {
        resolve();
        return;
  ...
  useEffect(() => {
    ...
    const gs = searchParams.get("groupSet");
    if (!tab) return;
    if (tab === "groups" && gs) {
      if (!groupSets.length) return;
      const found = groupSets.find((g) => g.id === gs);
      if (found) {
        setActiveTab(found.id);
        router.replace(`/admin/courses/${courseId}/people`, { scroll: false });
      } else {
        setActiveTab("groups");
      }
    } else if (tab === "groups") {
      if (groupSets.length > 0) {
        setActiveTab(groupSets[0].id);
        router.replace(`/admin/courses/${courseId}/people`, { scroll: false });
      } else {
        setActiveTab("groups");
      }
  ...
  const loadAllUsers = async () => {
    ...
    try {
      ...
      const enrolledEmails = new Set(people.map((p) => p.email.toLowerCase()));
      setAllUsers(
        (d.users ?? []).filter(
          (u: UserSuggestion & { role?: string }) =>
            !enrolledEmails.has(u.email.toLowerCase()) && u.role !== "ADMIN"
        )
      );
    } finally {
  ...
  useEffect(() => {
    if (addModal && addModalTab === "browse") void loadAllUsers();
  }, [addModal, addModalTab]);

  const loadSuggestions = async (q: string) => {
    setSuggLoading(true);
    setShowSugg(true);
    try {
      const res = await fetch(q ? `/api/admin/users?search=${encodeURIComponent(q)}` : "/api/admin/users");
      const d = await res.json();
      const enrolledEmails = new Set(people.map((p) => p.email.toLowerCase()));
      const chipEmails = new Set(chips.map((c) => c.email.toLowerCase()));
      setSuggestions(
        (d.users ?? [])
          .filter(
            (u: UserSuggestion) =>
              !enrolledEmails.has(u.email.toLowerCase()) && !chipEmails.has(u.email.toLowerCase())
          )
          .slice(0, 8)
      );
    } finally {
  ...
  useEffect(() => {
    if (!addModal) return;
    const t = setTimeout(() => {
      if (searchQ.length >= 1) void loadSuggestions(searchQ);
      else {
        setSuggestions([]);
        setShowSugg(false);
      }
  ...
  const addChipFromSugg = (u: UserSuggestion) => {
    setChips((p) => [
      ...p,
      {
  ...
  const handleKD = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if ((e.key === "Enter" || e.key === ",") && searchQ.trim()) {
      e.preventDefault();
      commitQ();
    }
    if (e.key === "Backspace" && searchQ === "" && chips.length > 0) {
  ...
  const handleAddPeople = async () => {
    ...
    if (!chips.length) {
      setAddError("Please add at least one person.");
      return;
    }
    ...
    for (const chip of chips) {
      try {
        const res = await fetch(`/api/admin/courses/${courseId}/people`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: chip.email, role: addRole }),
        });

        if (!res.ok) bad.push(chip.email);
        else {
          setChips((p) =>
            p.map((c) => (c.id === chip.id ? { ...c, status: "valid" } : c))
          );
        }
      } catch {
        bad.push(chip.email);
        setChips((p) =>
          p.map((c) => (c.id === chip.id ? { ...c, status: "invalid" } : c))
        );
      }
    }
    ...
    if (bad.length > 0) {
      setAddError(`Not found: ${bad.join(", ")}`);
      setChips((p) =>
        p.map((c) => (bad.includes(c.email) ? { ...c, status: "invalid" } : c))
      );
    } else {
      closeAddModal();
    }
  ...
  const handleBulkAdd = async () => {
    ...
    for (const u of allUsers.filter((u) => selectedIds.has(u.id))) {
      try {
        const res = await fetch(`/api/admin/courses/${courseId}/people`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ email: u.email, role: browseRole }),
        });
    ...
    await new Promise<void>((resolve) => {
      fetch(`/api/admin/courses/${courseId}/people`)
        ...
        .then((d) => {
          ...
          const enrolledEmails = new Set(up.map((p: { email: string }) => p.email.toLowerCase()));
          setAllUsers((p) => p.filter((u) => !enrolledEmails.has(u.email.toLowerCase())));
    ...
    setBulkAdding(false);
    if (failed.length > 0) setBulkError(`Failed: ${failed.join(", ")}`);
    else closeAddModal();
  };
  ...
  const removeUser = async (userId: string) => {
    ...
    startTransition(() => setPeople((p) => p.filter((x) => x.id !== userId)));
    setMenuOpenId(null);
  };

  const updatePersonRole = async (userId: string, nextRole: "Staff" | "Head") => {
  ...
  const addStudentToGroup = async (gsId: string, gId: string, userId: string) => {
    ...
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}/members`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId }),
    });
  ...
  const deleteGroup = async (gsId: string, gId: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}`, {
      method: "DELETE",
    });
    fetchGroupSets();
  ...
  const removeMember = async (gsId: string, gId: string, userId: string) => {
    await fetch(
      `/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}/members/${userId}`,
      { method: "DELETE" }
    );
    fetchGroupSets();
  ...
  const setLeader = async (gsId: string, gId: string, userId: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets/${gsId}/groups/${gId}/leader`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ userId }),
    });
  ...
  const resetGsModal = () => {
    ...
    setGroupStructure("Create groups later");
    setCreateGroupsNow(0);
    setAutoAssignLeader(false);
    setLeaderType("first");
  };
  ...
  const handleCreateGs = async () => {
    ...
    try {
      ...
      setSavingGroupSet(false);
    }
  };

  const deleteGs = async (id: string) => {
    await fetch(`/api/admin/courses/${courseId}/groupsets?groupSetId=${id}`, {
      method: "DELETE",
    });
    fetchGroupSets();
  ...
  const handleAddGroup = async () => {
    ...
    setSavingGroup(true);
    try {
      const res = await fetch(`/api/admin/courses/${courseId}/groupsets/${addGroupModal}/groups`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleMoveStudent = async () => {
    ...
    try {
      await fetch(
        `/api/admin/courses/${courseId}/groupsets/${movePanel.groupSetId}/groups/${movePanel.fromGroupId}/members/${movePanel.userId}`,
        { method: "DELETE" }
      );
      await fetch(
        `/api/admin/courses/${courseId}/groupsets/${movePanel.groupSetId}/groups/${moveToGroupId}/members`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: movePanel.userId,
            placement: movePlacement,
            relativeUserId: moveRelativeUserId || undefined,
          }),
        }
      );
      fetchGroupSets();
  ...
  const handleEditGs = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/courses/${courseId}/groupsets/${editGsModal.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editGsName.trim(),
          selfSignUp: editGsSelfSignUp,
          requireSameSection: editGsRequireSameSection,
          autoAssignLeader: editGsAutoAssignLeader,
          leaderType: editGsLeaderType,
          limitGroupMembers: editGsLimit,
        }),
      });
      if (res.ok) {
        await fetchGroupSetsAsync();
        setActiveTab(editGsModal.id);
        setEditGsModal(null);
      }
    } finally {
      setSavingEditGs(false);
    }
  };
  ...
  const handleCloneGs = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/courses/${courseId}/groupsets`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: cloneName.trim() || `(Clone) ${cloneModal.name}`,
          selfSignUp: cloneModal.selfSignUp,
          requireSameSection: cloneModal.requireSameSection,
          groupStructure: cloneModal.groupStructure,
          createGroupsNow: 0,
          limitGroupMembers: cloneModal.limitGroupMembers,
          autoAssignLeader: cloneModal.autoAssignLeader,
      ...
      if (res.ok) {
        ...
        for (const g of cloneModal.groups) {
          await fetch(`/api/admin/courses/${courseId}/groupsets/${newId}/groups`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  const filtered = people.filter((p) => {
    ...
    return (
      ...
      (roleFilter === "All Roles" || p.role.toLowerCase() === roleFilter.toLowerCase())
    );
  });

  const roleCounts = ALL_DISPLAY_ROLES.reduce<Record<string, number>>((a, r) => {
    a[r] = people.filter((p) => p.role.toLowerCase() === r.toLowerCase()).length;
    return a;
  }, {});

  const browseFiltered = allUsers.filter((u) => {
  ...
  const toggleAll = () => {
    if (allBrowseSel) {
      setSelectedIds((p) => {
        const s = new Set(p);
        browseFiltered.forEach((u) => s.delete(u.id));
        return s;
      });
    } else {
      setSelectedIds((p) => {
        const s = new Set(p);
        browseFiltered.forEach((u) => s.add(u.id));
        return s;
      });
    }
  };
  ...
  const unassigned = activeGs
    ? people.filter(
        (p) => !activeGs.groups.flatMap((g) => (g.members ?? []).map((m) => m.user.id)).includes(p.id)
      )
    : [];
      ...
      <div className="cpp-root">
        <div className="cpp-tabbar">
          <div className="cpp-tabs">
            {tabs.map((t) => (
              <button
                key={t.key}
                className={`cpp-tab${activeTab === t.key ? " active" : ""}`}
        ...
        {activeTab === "everyone" && (
          <>
            ...
            {loading ? (
              <p style={{ fontSize: 13, color: "#9ca3af", textAlign: "center", padding: "48px 0" }}>
                Loading...
              </p>
            ) : (
              <table className="cpp-table">
                <thead>
                  <tr>
                    ...
                    {/* Removed: SIS ID, Last Activity, Total Activity */}
                    {["Name", "Login ID", "Section", "Role"].map((h) => (
                      <th key={h}>{h}</th>
                    ))}
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={6} style={{ padding: "48px 0", textAlign: "center", fontSize: 13, color: "#9ca3af" }}>
                    ...
                    filtered.map((p, i) => (
                      <tr key={p.id} className={i % 2 === 0 ? "even" : "odd"}>
                        ...
                        <td className="name-col">
                          <span
                            className="cpp-name-link"
                            onClick={() => router.push(`/admin/courses/${courseId}/people/${p.id}`)}
                          >
                            {p.name}
                          </span>
                          {p.pronouns && (
                        ...
                        <td>{courseName}</td>
                        <td>
                          <span className="cpp-badge" style={roleBadge(p.role)}>
                        ...
                        <td style={{ position: "relative" }}>
                          ...
                          {menuOpenId === p.id && (
                            <div className="cpp-menu" style={{ right: 0, top: 36 }}>
                              {/* Only show the role option that is NOT the current role */}
                              {p.role !== "Staff" && (
                                <button
                                  className="cpp-menu-item"
                                  onClick={() => void updatePersonRole(p.id, "Staff")}
                                >
                                  Set as Staff
                                </button>
                              )}
                              {p.role !== "Head" && (
                                <button
                                  className="cpp-menu-item"
                                  onClick={() => void updatePersonRole(p.id, "Head")}
                                >
                                  Set as Head
                                </button>
                              )}
                              ...
                              <button
                                className="cpp-menu-item"
                                onClick={() => {
                                  router.push(`/admin/courses/${courseId}/people/${p.id}`);
                                  setMenuOpenId(null);
                                }}
                              >
                                User Details
                              ...
                              <div className="cpp-menu-divider" />
                              <button
                                className="cpp-menu-item danger"
        ...
        {isGroupsArea && groupSets.length === 0 && (
          <div style={{ marginTop: 24 }}>
            <h2 style={{ fontSize: 20, fontWeight: 800, color: "#7b1113", marginBottom: 12 }}>
              Student Groups
            </h2>
            <p
              style={{
            ...
            <p style={{ fontSize: 13, color: "#4a5568", lineHeight: 1.6, maxWidth: 820 }}>
              You can randomly assign students to groups of a specific size, or manually create and
              organize the groups. Once your groups are created, you can set assignments to be
              &quot;group submission&quot; assignments, which means each group will have one submission
        ...
        {activeGs && (
          <>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 8, marginBottom: 16 }}>
              <button
                className="cpp-btn-secondary"
                onClick={() => {
                  setNewGroupName("");
                  setNewGroupLimit(0);
                  setAddGroupModal(activeGs.id);
                }}
              >
                + Group
              </button>
              <div style={{ position: "relative" }}>
                ...
                {gsMenuOpen === activeGs.id && (
                  <div className="cpp-menu" style={{ right: 0, top: 38 }}>
                    <button
                      className="cpp-menu-item"
                      onClick={() => {
                        setEditGsModal(activeGs);
                        setEditGsName(activeGs.name);
                        setEditGsSelfSignUp(activeGs.selfSignUp);
                        setEditGsRequireSameSection(activeGs.requireSameSection);
                        setEditGsAutoAssignLeader(activeGs.autoAssignLeader);
                        setEditGsLeaderType(activeGs.leaderType);
                        setEditGsLimit(activeGs.limitGroupMembers ?? 0);
                        setGsMenuOpen(null);
                      }}
                    >
                      Edit
                    ...
                    <button
                      className="cpp-menu-item"
                      onClick={() => {
                        setCloneName(`(Clone) ${activeGs.name}`);
                        setCloneModal(activeGs);
                        setGsMenuOpen(null);
                      }}
                    >
                      Clone Group Set
                    ...
                    <div className="cpp-menu-divider" />
                    <button className="cpp-menu-item danger" onClick={() => void deleteGs(activeGs.id)}>
                      Delete
                    </button>
                  </div>
                )}
            ...
            <div className="cpp-groups-layout">
              <div className="cpp-unassigned-col">
                ...
                />
                {unassigned.length === 0 ? (
                  <div className="cpp-box-dashed cpp-empty-box">
                    No unassigned members. Add people to get started.
                  </div>
                ) : (
                  <div className="cpp-box">
                    {unassigned.map((p) => (
                      <div key={p.id} className="cpp-unassigned-row">
                        ...
                        <div style={{ position: "relative" }}>
                          <button
                            className="cpp-plus-btn"
                            onClick={() => setAddToGroupMenu(addToGroupMenu === p.id ? null : p.id)}
                          >
                            +
                          </button>
                          {addToGroupMenu === p.id && activeGs.groups.length > 0 && (
                            <div className="cpp-menu" style={{ right: 0, top: 28 }}>
                              <p
                                style={{
                                  fontSize: 11,
                                  fontWeight: 800,
                      ...
                      return (
                        <div key={g.id}>
                          <div className="cpp-group-row">
                            <button className="cpp-btn-icon" style={{ padding: 0 }} onClick={() => toggleGroup(g.id)}>
                              {expanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                            </button>
                            <span className="cpp-name-link" style={{ flex: 1 }}>
                              {g.name}
                            </span>
                            {leader && (
                              <span
                            ...
                            <div style={{ position: "relative" }}>
                              <button
                                className="cpp-btn-icon"
                                onClick={() => setGroupMenuOpen(groupMenuOpen === g.id ? null : g.id)}
                              >
                                <MoreVertical size={15} />
                              </button>
                              {groupMenuOpen === g.id && (
                                <div className="cpp-menu" style={{ right: 0, top: 28 }}>
                                  ...
                                  <button
                                    className="cpp-menu-item"
                                    onClick={() => {
                                      setEditGroupModal({ id: g.id, name: g.name, limit: 0 });
                                  ...
                                  <button
                                    className="cpp-menu-item danger"
                                    onClick={() => void deleteGroup(activeGs.id, g.id)}
                                  >
                                    Delete
                                  </button>
                                </div>
                                      ...
                                      <div key={m.user.id} className="cpp-member-card">
                                        ...
                                        <div style={{ position: "relative" }}>
                                          <button
                                            ...
                                            onClick={(e) => {
                                              e.stopPropagation();
                                              setMemberMenuOpen(memberMenuOpen === mk ? null : mk);
                                            }}
                                          >
                                          ...
                                          {memberMenuOpen === mk && (
                                            ...
                                            >
                                              <button
                                                className="cpp-menu-item"
                                                onClick={() => void removeMember(activeGs.id, g.id, m.user.id)}
                                              >
                                                Remove
                                              </button>
                                              {m.isLeader ? (
                                                <button
                                                  className="cpp-menu-item"
                                                  onClick={() => void removeLeader(activeGs.id, g.id)}
                                                >
                                                  Remove as Leader
                                                </button>
                                              ) : (
                                                <button
                                                  className="cpp-menu-item"
                                                  onClick={() => void setLeader(activeGs.id, g.id, m.user.id)}
                                                >
                                                  Set as Leader
                                                </button>
                                              )}
        ...
        {addModal && (
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) closeAddModal(); }}>
            <div className="cpp-modal" style={{ width: 620, maxHeight: "88vh" }}>
              <div className="cpp-modal-header">
                <h2 className="cpp-modal-title">Add People</h2>
                <button className="cpp-btn-icon" onClick={closeAddModal}>
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="cpp-subtabs">
                {[
                  { key: "search", label: "Search by Email" },
                  { key: "browse", label: "Browse & Select Users" },
                ].map((t) => (
                  <button
                    key={t.key}
                    className={`cpp-subtab${addModalTab === t.key ? " active" : ""}`}
                    onClick={() => setAddModalTab(t.key as "search" | "browse")}
                  >
                    {t.label}
                  </button>
                ))}
              ...
              {addModalTab === "search" && (
                <div className="cpp-modal-body">
                  <div style={{ marginBottom: 16 }}>
                    <p style={{ fontSize: 13, fontWeight: 700, color: "#2d3b45", marginBottom: 8 }}>
                      Add user(s) by
                    </p>
                  ...
                  <div style={{ marginBottom: 16 }}>
                    ...
                    >
                      {addBy === "email" ? "Email Addresses" : addBy === "loginid" ? "Login IDs" : "SIS IDs"}{" "}
                    ...
                    {showSugg && (suggestions.length > 0 || suggLoading) && (
                      <div className="cpp-sugg">
                        {suggLoading && !suggestions.length && (
                          ...
                          >
                            <Loader2 size={14} className="cpp-spin" />
                            Searching…
                          </div>
                        )}
                        {suggestions.map((u) => (
                          ...
                          >
                            ...
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 700, color: "#2d3b45", margin: 0 }}>
                                {u.name}
                              </p>
                    ...
                    <p style={{ fontSize: 11, color: "#9ca3af", marginTop: 4 }}>
                      Press Enter or , to add · Paste multiple emails at once
                    </p>
                    {addError && (
                      <p style={{ fontSize: 12, color: "#7b1113", marginTop: 4, fontWeight: 600 }}>
                        {addError}
                      </p>
                    )}
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 12 }}>
                    <div>
                      ...
                      <div style={{ position: "relative" }}>
                        <select
                          className="cpp-select"
                          value={addRole}
                          onChange={(e) => setAddRole(normalizeCourseRole(e.target.value))}
                          style={{ width: "100%" }}
                        >
                    ...
                    <div>
                      ...
                      <div style={{ position: "relative" }}>
                        <select className="cpp-select" style={{ width: "100%" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", flex: 1, overflow: "hidden" }}>
                  <div style={{ padding: "16px 20px 12px", flexShrink: 0 }}>
                    <div style={{ display: "flex", gap: 10 }}>
                      ...
                      <div style={{ position: "relative" }}>
                        <select
                          className="cpp-select"
                          value={browseRole}
                          onChange={(e) => setBrowseRole(normalizeCourseRole(e.target.value))}
                        >
                          {ENROLLMENT_ROLES.map((r) => (
                            <option key={r} value={r}>
                              {r}
                    ...
                    {selectedIds.size > 0 && (
                      <div className="cpp-banner">
                        <span style={{ fontSize: 13, color: "#7b1113", fontWeight: 600 }}>
                          {selectedIds.size} user{selectedIds.size !== 1 ? "s" : ""} selected — will
                          be added as <strong>{browseRole}</strong>
                        </span>
                        <button
                  ...
                  <div style={{ flex: 1, overflowY: "auto", borderTop: "1px solid #f0e4e4" }}>
                    {browseLoading ? (
                      ...
                      >
                        <Loader2 size={18} className="cpp-spin" />
                        Loading users…
                      </div>
                      ...
                      <table className="cpp-browse-table">
                        <thead className="cpp-browse-thead">
                          <tr>
                            <th style={{ width: 40, padding: "10px 14px" }}>
                              <input
                                type="checkbox"
                                checked={allBrowseSel}
                                ref={(el) => {
                                  if (el) el.indeterminate = someBrowseSel && !allBrowseSel;
                                }}
                                onChange={toggleAll}
                                style={{
                                  width: 14,
                        ...
                        <tbody>
                          {browseFiltered.map((u, i) => (
                            <tr
                              key={u.id}
                              className={`cpp-browse-tr${selectedIds.has(u.id) ? " sel" : i % 2 === 0 ? " even" : " odd"}`}
                            ...
                            >
                              <td style={{ padding: "10px 14px" }}>
                                <div
                                  style={{
                                    ...
                                    height: 18,
                                    borderRadius: "50%",
                                    border: `2px solid ${selectedIds.has(u.id) ? "#7b1113" : "#e5e7eb"}`,
                                    background: selectedIds.has(u.id) ? "#7b1113" : "#fff",
                                    display: "flex",
                                    alignItems: "center",
                                    justifyContent: "center",
                                  }}
                              ...
                              <td style={{ padding: "10px 12px" }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                  <Avatar name={u.name} image={u.image} size={30} />
                                  <span style={{ fontSize: 13, color: "#2d3b45", fontWeight: 600 }}>
                                    {u.name}
                                  </span>
                                </div>
                              </td>
                              <td style={{ padding: "10px 12px", fontSize: 13, color: "#6b7780" }}>
                                {u.email}
                              </td>
                            </tr>
                          ))}
                  ...
                  {bulkError && (
                    <p style={{ fontSize: 12, color: "#7b1113", padding: "6px 20px", fontWeight: 600 }}>
                      {bulkError}
                    </p>
                  )}
                </div>
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={closeAddModal}>
                  Cancel
                </button>
                {addModalTab === "search" ? (
                  <button
                    ...
                    disabled={adding || chips.length === 0}
                  >
                    {adding && <Loader2 size={13} className="cpp-spin" />}
                    {adding ? "Adding…" : "Next"}
                  </button>
                ) : (
                  <button
                  ...
                  >
                    {bulkAdding && <Loader2 size={13} className="cpp-spin" />}
                    {bulkAdding ? "Adding…" : `Add ${selectedIds.size > 0 ? selectedIds.size : ""} to Course`}
                  </button>
                )}
              </div>
            </div>
        ...
        {groupSetModal && (
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) setGroupSetModal(false); }}>
            <div className="cpp-modal" style={{ width: 560, maxHeight: "90vh" }}>
              <div className="cpp-modal-header">
                <h2 className="cpp-modal-title">Create Group Set</h2>
                <button className="cpp-btn-icon" onClick={() => setGroupSetModal(false)}>
                  <X size={18} />
                </button>
              </div>
              <div className="cpp-modal-body">
                <div style={{ display: "grid", gridTemplateColumns: "160px 1fr", gap: 12, alignItems: "center", marginBottom: 20 }}>
                  <label style={{ fontSize: 13, color: "#2d3b45", fontWeight: 600 }}>
                    Group Set Name <span style={{ color: "#c0392b" }}>*</span>
                  </label>
                  <input
                    className="cpp-input"
                    value={groupSetName}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: "#2d3b45" }}>
                      <input
                        ...
                        checked={selfSignUp}
                        onChange={(e) => {
                          setSelfSignUp(e.target.checked);
                          if (!e.target.checked) setRequireSameSection(false);
                        }}
                        style={accentM}
                      />
                      Allow self sign-up
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: selfSignUp ? "pointer" : "default", fontSize: 13, color: selfSignUp ? "#2d3b45" : "#9ca3af" }}>
                      <input
                        ...
                        onChange={(e) => setRequireSameSection(e.target.checked)}
                        disabled={!selfSignUp}
                        style={accentM}
                      />
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                      <div style={{ position: "relative" }}>
                        <select
                          ...
                          onChange={(e) => {
                            setGroupStructure(e.target.value);
                            setCreateGroupsNow(0);
                          }}
                          style={{ width: "100%" }}
                        >
                    ...
                    {[{ val: "first", label: "Set first member to join as group leader" }, { val: "random", label: "Set a random member as group leader" }].map((opt) => (
                      <label
                        ...
                        style={{
                          ...
                          alignItems: "center",
                          gap: 8,
                          cursor: autoAssignLeader ? "pointer" : "default",
                          fontSize: 13,
                          color: autoAssignLeader ? "#2d3b45" : "#9ca3af",
                          paddingLeft: 8,
                        }}
                      >
                        <input
                          ...
                          name="leaderType"
                          value={opt.val}
                          checked={leaderType === opt.val}
                          onChange={() => setLeaderType(opt.val)}
                          disabled={!autoAssignLeader}
                          style={accentM}
                        />
                        {opt.label}
                      </label>
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={() => setGroupSetModal(false)}>
                  Cancel
                </button>
                <button
                  className="cpp-btn-primary"
                  onClick={() => void handleCreateGs()}
                  disabled={savingGroupSet || !groupSetName.trim()}
                >
        ...
        {addGroupModal && (
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) setAddGroupModal(null); }}>
            <div className="cpp-modal" style={{ width: 440 }}>
              <div className="cpp-modal-header">
                <h2 className="cpp-modal-title">Add Group</h2>
                <button className="cpp-btn-icon" onClick={() => setAddGroupModal(null)}>
                  <X size={18} />
                </button>
              </div>
              <div className="cpp-modal-body">
                <div style={{ marginBottom: 16 }}>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#2d3b45", marginBottom: 6 }}>
                    Group Name <span style={{ color: "#c0392b" }}>*</span>
                  </label>
                  <input
                    className="cpp-input"
                    value={newGroupName}
                    ...
                    placeholder="Name"
                    style={{ width: "100%", boxSizing: "border-box", height: 34 }}
                  />
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 700, color: "#2d3b45", marginBottom: 6 }}>
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={() => setAddGroupModal(null)}>
                  Cancel
                </button>
                <button
                  className="cpp-btn-primary"
        ...
        {editGroupModal && (
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) setEditGroupModal(null); }}>
            <div className="cpp-modal" style={{ width: 380 }}>
              <div className="cpp-modal-header">
                <h2 className="cpp-modal-title">Edit Group</h2>
                <button className="cpp-btn-icon" onClick={() => setEditGroupModal(null)}>
                  <X size={18} />
                </button>
              </div>
              <div className="cpp-modal-body">
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={() => setEditGroupModal(null)}>
                  Cancel
                </button>
                <button
                  className="cpp-btn-primary"
            ...
            return (
              <div style={{ position: "fixed", inset: 0, zIndex: 50, display: "flex", justifyContent: "flex-end" }}>
                ...
                <div className="cpp-side-panel">
                  <div className="cpp-modal-header">
                    <h2 style={{ fontSize: 16, fontWeight: 800, color: "#2d3b45", margin: 0 }}>
                      Move Member
                    </h2>
                    <button className="cpp-btn-icon" onClick={() => setMovePanel(null)}>
                      <X size={16} />
                    </button>
                  </div>
                  <div style={{ padding: 20, flex: 1, display: "flex", flexDirection: "column", gap: 18 }}>
                    <div>
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        >
                          {others.length === 0 ? (
                            <option value="">No other groups</option>
                          ) : (
                            others.map((g) => (
                  ...
                  <div className="cpp-modal-footer">
                    <button className="cpp-btn-secondary" onClick={() => setMovePanel(null)}>
                      Cancel
                    </button>
                    <button
                      className="cpp-btn-primary"
                      onClick={() => void handleMoveStudent()}
                      disabled={movingStudent || !moveToGroupId}
                    >
                      {movingStudent && <Loader2 size={13} className="cpp-spin" />}
                      Move
                    </button>
                  </div>
                </div>
          ...
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) setEditGsModal(null); }}>
            <div className="cpp-modal" style={{ width: 560, maxHeight: "90vh" }}>
              <div className="cpp-modal-header">
                ...
                <button className="cpp-btn-icon" onClick={() => setEditGsModal(null)}>
                  <X size={18} />
                </button>
              </div>
              <div className="cpp-modal-body">
                <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
                  <label style={{ fontSize: 13, color: "#2d3b45", fontWeight: 600, width: 160, flexShrink: 0 }}>
                    Group Set Name
                  </label>
                  <input
                    className="cpp-input"
                    value={editGsName}
                    onChange={(e) => setEditGsName(e.target.value)}
                ...
                <div style={{ borderTop: "1px solid #f0e4e4", paddingTop: 16, marginBottom: 16, display: "flex", flexDirection: "column", gap: 10 }}>
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: "#2d3b45" }}>
                    <input
                      type="checkbox"
                      checked={editGsSelfSignUp}
                      onChange={(e) => {
                        setEditGsSelfSignUp(e.target.checked);
                        if (!e.target.checked) {
                          setEditGsRequireSameSection(false);
                          setEditGsLimit(0);
                        }
                      }}
                      style={accentM}
                  ...
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: editGsSelfSignUp ? "pointer" : "default", fontSize: 13, color: editGsSelfSignUp ? "#2d3b45" : "#9ca3af" }}>
                    <input
                      type="checkbox"
                      checked={editGsRequireSameSection}
                      onChange={(e) => setEditGsRequireSameSection(e.target.checked)}
                      disabled={!editGsSelfSignUp}
                      style={accentM}
                    />
                    Require group members to be in the same section
                  </label>
                ...
                <div style={{ borderTop: "1px solid #f0e4e4", paddingTop: 16, display: "flex", flexDirection: "column", gap: 10 }}>
                  ...
                  <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer", fontSize: 13, color: "#2d3b45" }}>
                    <input
                      type="checkbox"
                      checked={editGsAutoAssignLeader}
                      onChange={(e) => setEditGsAutoAssignLeader(e.target.checked)}
                      style={accentM}
                    />
                    Automatically assign a group leader
                  </label>
                  {[{ val: "first", label: "Set first member to join as group leader" }, { val: "random", label: "Set a random member as group leader" }].map((opt) => (
                    <label
                      ...
                      style={{
                        ...
                        alignItems: "center",
                        gap: 8,
                        cursor: editGsAutoAssignLeader ? "pointer" : "default",
                        fontSize: 13,
                        color: editGsAutoAssignLeader ? "#2d3b45" : "#9ca3af",
                        paddingLeft: 8,
                      }}
                    >
                      <input
                        ...
                        name="editLeaderType"
                        value={opt.val}
                        checked={editGsLeaderType === opt.val}
                        onChange={() => setEditGsLeaderType(opt.val)}
                        disabled={!editGsAutoAssignLeader}
                        style={accentM}
                      />
                      {opt.label}
                    </label>
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={() => setEditGsModal(null)}>
                  Cancel
                </button>
                <button
                  className="cpp-btn-primary"
                  onClick={() => void handleEditGs()}
                  disabled={savingEditGs || !editGsName.trim()}
                >
                  {savingEditGs && <Loader2 size={13} className="cpp-spin" />}
                  Save
                </button>
              </div>
            </div>
          ...
          <div className="cpp-overlay" onClick={(e) => { if (e.target === e.currentTarget) setCloneModal(null); }}>
            <div className="cpp-modal" style={{ width: 440 }}>
              <div className="cpp-modal-header">
                ...
                <button className="cpp-btn-icon" onClick={() => setCloneModal(null)}>
                  <X size={18} />
                </button>
              </div>
              <div className="cpp-modal-body">
              ...
              <div className="cpp-modal-footer">
                <button className="cpp-btn-secondary" onClick={() => setCloneModal(null)}>
                  Cancel
                </button>
                <button
                  className="cpp-btn-primary"
                  onClick={() => void handleCloneGs()}
                  disabled={submittingClone || !cloneName.trim()}
                >
                  {submittingClone && <Loader2 size={13} className="cpp-spin" />}
                  Submit
                </button>
              </div>
            </div>
