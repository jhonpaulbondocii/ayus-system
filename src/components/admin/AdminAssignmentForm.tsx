...
const SUBMISSION_ENTRY_TYPES = ["File Upload", "Text Entry", "Website URL", "Media Recording"];
...
interface SubmissionEntry {
  id: number; label: string; required: boolean; type: string;
  allowedFileTypes?: string[]; maxFiles?: number | null;
  maxFileSizeValue?: number; maxFileSizeUnit?: "KB" | "MB";
}
...
export interface AdminAssignmentFormProps {
  courseId: string;
  assignmentId?: string;       // undefined = create mode, string = edit mode
  initialName?: string;        // create only (from searchParams)
  initialPoints?: string;      // create only
  initialGroup?: string;       // create only
...
function SubmissionEntryCard({ entry, index, canRemove, onRemove, onUpdate }: {
  entry: SubmissionEntry; index: number; canRemove: boolean;
  onRemove: () => void;
...
export default function AdminAssignmentForm({
  ...
  const [initialDescKey, setInitialDescKey] = useState(0);
  ...
  const [submissionEntries, setSubmissionEntries] = useState<SubmissionEntry[]>([
    { id: 1, label: "", required: false, type: "File Upload", allowedFileTypes: [], maxFiles: 1, maxFileSizeValue: 1, maxFileSizeUnit: "MB" },
  ]);
  ...
  // ── Fetch existing assignment (edit mode only) ──────────────────────────
  useEffect(() => {
    if (!isEdit || !courseId || !assignmentId) return;
    fetch(`/api/admin/courses/${courseId}/assignments/${assignmentId}`)
      ...
      .then(d => {
        ...
        setInitialDescKey(k => k + 1);

        // Parse submissionEntries — GET API already normalizes them to array of objects
        console.log("RAW submissionEntries from API:", JSON.stringify(a.submissionEntries));
        const rawEntries = (() => {
          const raw = a.submissionEntries;
        ...
        console.log("PARSED rawEntries:", rawEntries);
        setSubmissionEntries(
          rawEntries.length > 0
            ? rawEntries.map((raw: unknown) => {
                const blank: SubmissionEntry = { id: Date.now() + Math.random(), label: "", required: false, type: "File Upload", allowedFileTypes: [], maxFiles: 1, maxFileSizeValue: 1, maxFileSizeUnit: "MB" };
                if (!raw || typeof raw !== "object") return blank;
                const o = raw as Record<string, unknown>;
                // Handle nested JSON string (legacy format)
                const resolved = typeof o.label === "string" && o.label.trim().startsWith("{")
                  ? (() => { try { return { ...o, ...JSON.parse(o.label as string) }; } catch { return o; } })()
                  : o;
                return {
                  id: typeof resolved.id === "number" ? resolved.id : Date.now() + Math.random(),
                  label: typeof resolved.label === "string" ? resolved.label : "",
                  required: typeof resolved.required === "boolean" ? resolved.required : false,
                  type: typeof resolved.type === "string" && SUBMISSION_ENTRY_TYPES.includes(resolved.type) ? resolved.type : "File Upload",
                  allowedFileTypes: Array.isArray(resolved.allowedFileTypes) ? resolved.allowedFileTypes as string[] : [],
                  maxFiles: typeof resolved.maxFiles === "number" ? resolved.maxFiles : 1,
                  maxFileSizeValue: typeof resolved.maxFileSizeValue === "number" ? resolved.maxFileSizeValue : 1,
                  maxFileSizeUnit: (resolved.maxFileSizeUnit === "KB" ? "KB" : "MB") as "KB" | "MB",
                };
              })
            : [{ id: Date.now(), label: "", required: false, type: "File Upload", allowedFileTypes: [], maxFiles: 1, maxFileSizeValue: 1, maxFileSizeUnit: "MB" }]
        );
        setAssignRows([{
          id: 1,
          assignees: (Array.isArray(a.assignees) && a.assignees.length > 0) ? a.assignees : ["Everyone"],
          dueDate: isoToDate(a.dueDate),
          dueTime: isoToTime(a.dueDate) || "11:59 PM",
          availableFrom: isoToDate(a.availableFrom),
          availableFromTime: isoToTime(a.availableFrom) || "12:00 AM",
          until: isoToDate(a.availableUntil),
          untilTime: isoToTime(a.availableUntil) || "11:59 PM",
        }]);

        setLoading(false);
      })
      .catch(() => setLoading(false));
  }, [courseId, assignmentId, isEdit]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/courses/${courseId}/groupsets`)
      .then(r => r.json()).then(d => setGroupSets(d.groupSets ?? [])).catch(() => {});
  }, [courseId]);
  ...
  const updateSubmissionEntry = (id: number, field: keyof SubmissionEntry, value: string | boolean | string[] | number | null) =>
  ...
  const handleSave = async (publish: boolean) => {
    ...
    console.log("SAVE PAYLOAD DEBUG:", {
      name, points, group, submissionType, submissionEntries,
      displayGradeAs, doNotCount, description,
    });
    setSaving(true);
    try {
      const row = assignRows[0];
      const entriesPayload = submissionEntries.map(entry => ({
        id: entry.id,
        label: entry.label,
        required: entry.required,
        type: entry.type,
        allowedFileTypes: (entry.type === "File Upload" || entry.type === "Media Recording") ? normalizeFileTypes(entry.allowedFileTypes) : [],
        maxFiles: (entry.type === "File Upload" || entry.type === "Media Recording") ? entry.maxFiles ?? 1 : null,
        maxFileSizeValue: (entry.type === "File Upload" || entry.type === "Media Recording") ? entry.maxFileSizeValue ?? 1 : null,
        maxFileSizeUnit: (entry.type === "File Upload" || entry.type === "Media Recording") ? entry.maxFileSizeUnit ?? "MB" : null,
      }));

      let res: Response;

      if (isEdit) {
        // PATCH — use submissionEntries directly
        res = await fetch(`/api/admin/courses/${courseId}/assignments/${assignmentId}`, {
          method: "PATCH",
          ...
          body: JSON.stringify({
            ...
            submissionEntries: entriesPayload,
        ...
        // POST — use submissionEntries (API accepts both submissionEntries and onlineEntryOptions)
        res = await fetch(`/api/admin/courses/${courseId}/assignments`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            title: name.trim(), description, points: parseFloat(points) || 0,
            ...
            submissionEntries: entriesPayload,
            submissionAttempts, allowedAttempts: submissionAttempts === "Limited" ? allowedAttempts : null,
            doNotCount: false, isGroupAssignment, groupSetId: groupSet || null,
            notifyUsers: publish ? notifyUsers : false,
          }),
        });
      }
      ...
      if (res.ok) {
        ...
        onSaved((data as { assignment?: { id?: string } })?.assignment?.id);
      } else {
        const data = await res.json().catch(() => ({}));
  ...
  if (!mounted || (isEdit && loading)) return (
              ...
              {isGroupAssignment && (
                <div className="pl-2 space-y-2">
                  ...
                  <div>
                    ...
                    <select value={groupSet} onChange={e => { if (e.target.value === "__new__") { setGroupSetName(""); setSelfSignUp(false); setRequireSameSection(false); setGroupStructure("Create groups later"); setShowGroupSetModal(true); } else setGroupSet(e.target.value); }}
