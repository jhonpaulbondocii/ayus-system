...
import {
  ...
  Upload, Users, Trash2, FileText, AlertCircle,
} from "lucide-react";
import {
...
export function validateFileTypes(
  ...
  for (const file of files) {
    ...
    if (!allowedFileTypes.map(t => t.toLowerCase()).includes(ext)) {
      errors.push(
        `"${file.name}" is not allowed. Accepted: ${allowedFileTypes.map(t => t.toUpperCase()).join(", ")}.`
      );
    }
  }
...
export function getAssignmentFileRestrictions(assignment: AssignmentWithRole): {
  hasRestrictions: boolean;
  allowedFileTypes: string[];
  maxFiles: number;
} {
  const entries: SubmissionEntryExtended[] =
    (assignment as AssignmentWithSubmissionEntries).submissionEntries ?? [];
  const fileEntry = entries.find(e => e.type === "File Upload");
  const allowedFileTypes = fileEntry?.allowedFileTypes ?? [];
  const maxFiles = fileEntry?.maxFiles ?? 1;
  return {
    hasRestrictions: allowedFileTypes.length > 0,
    allowedFileTypes,
    maxFiles,
  };
}
...
function getAssignmentFileUploadEntries(assignment: AssignmentWithRole): SubmissionEntryExtended[] {
  const entries = (assignment as AssignmentWithSubmissionEntries).submissionEntries ?? [];
  const fileEntries = entries.filter(e => e.type === "File Upload");

  if (fileEntries.length > 0) return fileEntries;

  const { allowedFileTypes, maxFiles } = getAssignmentFileRestrictions(assignment);
  return [{
...
/* ─────────────────────────────────────────────────────────────────────────────
   BUILD SUBMITTING LABEL — shows specific file types when restricted
───────────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────────
   FILE RESTRICTION BADGE COMPONENT
───────────────────────────────────────────────────────────────────────────── */
function FileRestrictionBadge({ allowedTypes }: { allowedTypes: string[] }) {
...
function formatAllowedFileTypes(allowedFileTypes: string[]): string {
  return allowedFileTypes.map(t => t.toUpperCase()).join(", ");
}

function buildRequiredFileUploadLabel(allowedFileTypes: string[]): string {
  if (!allowedFileTypes || allowedFileTypes.length === 0) return "File Upload";
  if (allowedFileTypes.length === 1) return `${allowedFileTypes[0].toUpperCase()} File Upload`;
  return `File Upload (${formatAllowedFileTypes(allowedFileTypes)})`;
}

/* ─────────────────────────────────────────────────────────────────────────────
   FILE UPLOAD SECTION FOR SUBMITTER (with validation)
───────────────────────────────────────────────────────────────────────────── */
function SubmitterFileUploadSection({
  ...
  const setInputRef = (entryKey: string) => (el: HTMLInputElement | null) => {
    inputRefs.current[entryKey] = el;
  };

  const clearInputValue = (entryKey: string) => {
    const input = inputRefs.current[entryKey];
    if (input) input.value = "";
  };

  const handleFilesForEntry = (entry: SubmissionEntryExtended, files: File[]) => {
    const entryKey = String(entry.id);
    const allowedFileTypes = entry.allowedFileTypes ?? [];
    ...
    if (files.length > maxFiles) {
      setFileErrorsByEntry(prev => ({
        ...prev,
        [entryKey]: [`You can only upload up to ${maxFiles} file${maxFiles !== 1 ? "s" : ""}.`],
      }));
      setSelectedFilesByEntry(prev => ({ ...prev, [entryKey]: [] }));
      clearInputValue(entryKey);
      return;
    }

    const { valid, errors } = validateFileTypes(files, allowedFileTypes);
    if (!valid) {
  ...
  return (
    <div className="mt-5 space-y-5">
      {fileEntries.map((entry, index) => {
        const entryKey = String(entry.id);
        const allowedFileTypes = entry.allowedFileTypes ?? [];
        const hasRestrictions = allowedFileTypes.length > 0;
        const maxFiles = entry.maxFiles ?? 1;
        const requiredUploadLabel = entry.label?.trim()
          ? entry.label.trim()
          : buildRequiredFileUploadLabel(allowedFileTypes);
        ...
        return (
          <div key={entryKey} className="rounded-xl border border-gray-200 bg-white overflow-hidden">
            <div className="flex items-start justify-between gap-3 px-4 py-3" style={{ background: "#fffafa", borderBottom: "1px solid #f3e1e1" }}>
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-sm font-black text-gray-900">
                    {fileEntries.length > 1 ? `${index + 1}. ` : ""}{requiredUploadLabel}
                  </p>
                  {entry.required && (
                    <span
                ...
                <p className="text-xs text-gray-500 mt-1">
                  {hasRestrictions
                    ? `Upload ${formatAllowedFileTypes(allowedFileTypes)} file${allowedFileTypes.length !== 1 ? "s" : ""} only.`
                    : "All file types are accepted."}
                </p>
                {hasRestrictions && <FileRestrictionBadge allowedTypes={allowedFileTypes} />}
              </div>
              ...
              {hasRestrictions && (
                ...
                >
                  {formatAllowedFileTypes(allowedFileTypes)} only
                </span>
              )}
            </div>
            ...
            <div className="p-4 space-y-3">
              <div
                className="relative border-2 border-dashed rounded-lg px-4 py-7 flex flex-col items-center justify-center gap-2 cursor-pointer transition-colors"
                style={{
                  borderColor: fileErrors.length > 0 ? "#ef4444" : "#d1d5db",
                  background: fileErrors.length > 0 ? "#fef2f2" : "#fafafa",
                }}
                onClick={() => inputRefs.current[entryKey]?.click()}
                onDragOver={e => e.preventDefault()}
                onDrop={e => {
                  e.preventDefault();
                  handleFilesForEntry(entry, Array.from(e.dataTransfer.files));
                }}
              >
                <input
                  ref={setInputRef(entryKey)}
                  type="file"
                  accept={acceptAttr}
                  multiple={maxFiles > 1}
                  className="sr-only"
                ...
                <Upload
                  size={22}
                  style={{ color: fileErrors.length > 0 ? "#ef4444" : MAROON }}
                />
                <p className="text-xs font-semibold text-gray-700 text-center">
                  Drag a file here, or <span style={{ color: MAROON }}>choose {requiredUploadLabel}</span>
                </p>
                <p className="text-[11px] text-gray-400 text-center">
                  {hasRestrictions
                    ? `Accepted: ${allowedFileTypes.map(t => `.${t.toUpperCase()}`).join(", ")}`
                    : "Any file format"}
              ...
              {selectedFiles.length > 0 && (
                <div className="space-y-1.5">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">
                    Selected Files for {requiredUploadLabel}
                  </p>
                  {selectedFiles.map((f, i) => (
                    ...
                    >
                      <div className="flex items-center gap-2 min-w-0">
                        <FileText size={13} style={{ color: MAROON, flexShrink: 0 }} />
                      ...
                      <button
                        type="button"
                        onClick={e => { e.stopPropagation(); removeFile(entryKey, i); }}
...
const RESPONSIVE_CSS = `
  .cat-toolbar { display:flex; align-items:center; justify-content:space-between; padding:10px 20px; border-bottom:1px solid #e5e7eb; gap:8px; flex-wrap:wrap; }
  .cat-toolbar-right { display:flex; align-items:center; gap:8px; flex-wrap:wrap; }

  .cat-row { display:flex; align-items:center; gap:10px; padding:12px 16px; border-bottom:1px solid #e5e7eb; background:#fff; cursor:pointer; transition:background .1s; position:relative; }
  .cat-row:hover { background:#fdf8f8; }
  .cat-row-meta { display:flex; align-items:center; gap:6px; flex-wrap:wrap; }

  .cat-detail-header { display:flex; align-items:center; justify-content:space-between; padding:10px 20px; border-bottom:1px solid #e5e7eb; background:#fff; flex-wrap:wrap; gap:8px; flex-shrink:0; }
  .cat-detail-actions { display:flex; align-items:center; gap:6px; flex-wrap:wrap; }

  .cat-detail-body { display:flex; flex:1; overflow:hidden; }
  .cat-detail-main { flex:1; overflow-y:auto; padding:24px 32px; }
  .cat-detail-sidebar { width:220px; border-left:1px solid #e5e7eb; flex-shrink:0; overflow-y:auto; }

  .cat-create-grid { display:grid; grid-template-columns:200px 1fr; align-items:start; gap:12px 16px; max-width:720px; }
  .cat-create-label { font-size:12px; color:#374151; text-align:right; padding-top:8px; }

  .cat-assign-panel { position:fixed; top:0; right:0; height:100%; z-index:50; background:#fff; box-shadow:-4px 0 24px rgba(0,0,0,.12); border-left:1px solid #e5e7eb; display:flex; flex-direction:column; width:380px; }

  .cat-schedule-table { width:100%; font-size:13px; overflow-x:auto; display:block; }

  .cat-submitter-page { display:flex; flex-direction:column; height:100%; background:#fff; font-family:${FONT}; }
  .cat-submitter-topbar { display:flex; align-items:center; justify-content:space-between; padding:10px 20px; border-bottom:1px solid #e5e7eb; flex-shrink:0; flex-wrap:wrap; gap:8px; }
  .cat-submitter-content { flex:1; overflow-y:auto; padding:32px 40px; max-width:900px; width:100%; }
  .cat-submitter-title { font-size:24px; font-weight:900; color:#111827; margin-bottom:12px; }
  .cat-submitter-meta { display:flex; align-items:center; gap:0; border-bottom:1px solid #e5e7eb; padding-bottom:14px; margin-bottom:20px; flex-wrap:wrap; }
  .cat-submitter-meta-item { display:flex; align-items:center; gap:4px; padding-right:16px; margin-right:16px; border-right:1px solid #e5e7eb; font-size:13px; color:#111827; }
  .cat-submitter-meta-item:last-child { border-right:none; margin-right:0; padding-right:0; }
  .cat-submitter-meta-label { font-weight:700; }
  .cat-submitter-desc { font-size:14px; color:#374151; line-height:1.7; }
  .cat-submitter-desc p { margin:0 0 8px; }
  .cat-submitter-desc strong, .cat-submitter-desc b { font-weight:700; color:#111827; }
  .cat-submitter-desc ul, .cat-submitter-desc ol { padding-left:20px; margin:0 0 8px; }
  .cat-submitter-desc li { margin-bottom:4px; }
  .cat-submitter-desc a { color:#7b1113; text-decoration:underline; }

  .cat-publisher-bar { display:flex; align-items:center; gap:10px; padding:10px 20px; border-bottom:1px solid #e5e7eb; background:#fafafa; flex-shrink:0; }

  .cat-view-toggle { display:flex; align-items:center; border:1px solid #e5e7eb; border-radius:8px; overflow:hidden; }
  .cat-view-toggle button { padding:6px 14px; font-size:12px; font-weight:700; border:none; background:transparent; cursor:pointer; transition:background .15s,color .15s; white-space:nowrap; }
  .cat-view-toggle button.active { background:${MAROON}; color:#fff; }
  .cat-view-toggle button:not(.active) { color:#6b7280; }
  ...
  .cat-section-divider {
    height:1px; background:#e5e7eb; margin:0 0 0 0;
  }

  @media (max-width: 768px) {
    .cat-toolbar { padding:10px 14px; }
    .cat-toolbar-right { width:100%; justify-content:flex-end; }
    .cat-row { padding:10px 12px; gap:8px; }
    .cat-detail-header { padding:10px 14px; }
    .cat-detail-actions .cat-action-label { display:none; }
    .cat-detail-body { flex-direction:column; }
    .cat-detail-main { padding:16px; }
    .cat-detail-sidebar { width:100%; border-left:none; border-top:1px solid #e5e7eb; }
    .cat-create-grid { grid-template-columns:1fr; }
    .cat-create-label { text-align:left; padding-top:0; }
    .cat-assign-panel { width:100%; top:auto; bottom:0; height:80vh; border-left:none; border-top:1px solid #e5e7eb; box-shadow:0 -4px 24px rgba(0,0,0,.12); border-radius:16px 16px 0 0; }
    .cat-submitter-content { padding:20px 16px; }
    .cat-submitter-title { font-size:18px; }
    .cat-submitter-topbar { padding:10px 14px; }
    .cat-submitter-meta { gap:0; }
    .cat-submitter-meta-item { font-size:12px; padding-right:10px; margin-right:10px; }
    .cat-publisher-bar { padding:8px 14px; }
  }

  @media (max-width: 480px) {
    .cat-row { padding:8px 10px; }
    .cat-submitter-content { padding:14px 12px; }
    .cat-submitter-meta-item { border-right:none; margin-right:0; padding-right:0; width:100%; }
  }
`;
...
function PublisherBar({
  ...
  return (
    <div className="cat-publisher-bar">
...
function QuickEditModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 px-4 sm:px-0" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-lg shadow-2xl w-full sm:w-[480px] border border-gray-200 overflow-hidden" onClick={e => e.stopPropagation()} style={{ fontFamily: FONT }}>
              ...
              <div>
                ...
                <div className="relative">
                  <select value={dueTime} onChange={e => setDueTime(e.target.value)}
                    className="h-9 border border-gray-300 rounded px-3 text-xs bg-white outline-none appearance-none pr-7" style={{ minWidth: 120 }}
                    onFocus={e => (e.currentTarget.style.borderColor = MAROON)} onBlur={e => (e.currentTarget.style.borderColor = "#d1d5db")}>
...
function AssignToPanel({
  ...
  return (
    <>
      ...
      <div className="cat-assign-panel" style={{ fontFamily: FONT }}>
...
function AddGroupModal({ onClose, onSave, saving }: { onClose: () => void; onSave: (name: string) => void; saving: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 px-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-lg shadow-2xl w-full sm:w-[420px] border border-gray-200 overflow-hidden" onClick={e => e.stopPropagation()} style={{ fontFamily: FONT }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
        ...
        <div className="px-6 py-6">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <input autoFocus value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === "Enter" && name.trim() && onSave(name.trim())} placeholder="e.g., Essay Group 1"
              className="flex-1 min-w-[160px] h-9 border border-gray-300 rounded px-3 text-sm outline-none"
              onFocus={e => (e.currentTarget.style.borderColor = MAROON)} onBlur={e => (e.currentTarget.style.borderColor = "#d1d5db")} />
          </div>
        </div>
...
function DeleteGroupModal({ groupName, assignmentCount, otherGroups, onClose, onDelete }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 px-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-lg shadow-2xl w-full sm:w-[460px] border border-gray-200 overflow-hidden" onClick={e => e.stopPropagation()} style={{ fontFamily: FONT }}>
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
...
function AssignmentRowDropdown({ assignment, onAction, onClose }: {
  assignment: AssignmentWithRole;
  onAction: (action: AssignmentDropdownAction, a: AssignmentWithRole) => void;
  onClose: () => void;
}) {
...
function GroupDropdown({ onEdit, onDelete, onClose, isLastGroup }: {
  onEdit: () => void;
  onDelete: () => void;
  onClose: () => void;
  isLastGroup?: boolean;
...
function AdminAssignmentRow({ a, onView, onEdit, onDuplicate, onAssignTo, onSpeedGrader }: {
  ...
  onSpeedGrader: (a: AssignmentWithRole) => void;
}) {
  const [menuOpen, setMenuOpen] = useState(false);
  const now = new Date();
  const isClosed = a.availableUntil && now > new Date(a.availableUntil);
  const due = fmtDue(a.dueDate);
  const handleAction = (action: AssignmentDropdownAction, assignment: AssignmentWithRole) => {
  ...
  return (
    <div className="cat-row" onClick={() => onView(a)}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="cat-row-meta text-xs text-gray-500 mt-0.5">
...
function AdminAssignmentGroupSection({ title, items, onAddAssignment, onView, onEdit, onDuplicate, onAssignTo, onSpeedGrader, onEditGroup, onDeleteGroup, isLastGroup }: {
  title: string;
...
/* ─────────────────────────────────────────────────────────────────────────────
   HEAD SUBMITTER DETAIL
───────────────────────────────────────────────────────────────────────────── */
function HeadSubmitterDetail({
  assignment, courseId, onBack, currentUserId,
}: {
  assignment: AssignmentWithRole; courseId: string; onBack: () => void;
  currentUserId?: string | null;
}) {
  const router = useRouter();
  const sub = assignment.submissions?.[0];
  const now = new Date();
  const isAvailableFrom = assignment.availableFrom && now < new Date(assignment.availableFrom);
  const isClosed = assignment.availableUntil && now > new Date(assignment.availableUntil);

  // ── FIX: get file restrictions FIRST, then build label ──
  const { hasRestrictions, allowedFileTypes, maxFiles } = getAssignmentFileRestrictions(assignment);
  ...
  return (
    <div className="cat-submitter-page">
      <div className="cat-submitter-topbar">
        <button onClick={onBack} className="flex items-center gap-1.5 text-sm font-semibold hover:underline" style={{ color: MAROON }}>
        ...
        {!isAvailableFrom && (
          <button
            ...
            disabled={!!isClosed && !sub?.submittedAt}
            className="h-9 px-5 text-sm font-bold text-white rounded-lg hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity"
            style={{ background: MAROON }}
          >
            {sub?.submittedAt ? "View Submission" : "Start Assignment"}
          </button>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        <div className="cat-submitter-content">
          <h1 className="cat-submitter-title">{assignment.title}</h1>
          <div className="cat-submitter-meta">
            <div className="cat-submitter-meta-item">
              <span className="cat-submitter-meta-label">Points</span>
              <span>{assignment.points}</span>
            </div>
            {/* ── FIX: now shows e.g. "File Upload (PDF, DOCX)" ── */}
            <div className="cat-submitter-meta-item">
              <span className="cat-submitter-meta-label">Submitting</span>
              <span>{submittingLabel}</span>
            </div>
            <div className="cat-submitter-meta-item">
              <span className="cat-submitter-meta-label">Attempts</span>
              <span>{attemptCount}</span>
            </div>
            {fmtDue(assignment.dueDate) && (
              <div className="cat-submitter-meta-item">
                <span className="cat-submitter-meta-label">Due</span>
                <span>{fmtDue(assignment.dueDate)}</span>
              </div>
            )}
          </div>

          {/* FILE RESTRICTIONS NOTICE */}
          {hasRestrictions && !sub?.submittedAt && (
            <div
              className="flex items-start gap-3 px-4 py-3 rounded-lg mb-5"
              style={{ background: "#fef2f2", border: `1px solid #f0c0c0` }}
            >
              <FileText size={15} style={{ color: MAROON, flexShrink: 0, marginTop: 1 }} />
              <div>
                <p className="text-sm font-bold mb-1" style={{ color: MAROON }}>
                  {buildRequiredFileUploadLabel(allowedFileTypes)} Required
                </p>
                <p className="text-xs text-gray-600 mb-2">
                  This assignment only accepts the following file format{allowedFileTypes.length !== 1 ? "s" : ""}:
                </p>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {allowedFileTypes.map(t => (
                    <span
                      key={t}
                      className="px-2.5 py-1 rounded text-xs font-black uppercase"
                      style={{ background: MAROON, color: "#fff", letterSpacing: "0.05em" }}
                    >
                      .{t}
                    </span>
                  ))}
                ...
                <p className="text-[11px] text-gray-500">
                  Max {maxFiles} file{maxFiles !== 1 ? "s" : ""}. Uploading any other format will be rejected.
                </p>
              </div>
            </div>
          )}
          ...
          {isAvailableFrom && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg mb-5 text-sm font-semibold"
              style={{ background: "#fffbeb", border: "1px solid #fde68a", color: "#92400e" }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><line x1="12" y1="8" x2="12" y2="12" /><line x1="12" y1="16" x2="12.01" y2="16" /></svg>
              <span>Opens {fmtDate(assignment.availableFrom)}</span>
            </div>
          )}
          ...
          {isClosed && (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg mb-5 text-sm font-semibold"
              ...
              <span>Closed {fmtDate(assignment.availableUntil)}</span>
            </div>
          )}

          {sub?.submittedAt ? (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg mb-5 text-sm font-semibold flex-wrap"
              style={{ background: "#f0fdf4", border: "1px solid #86efac", color: "#15803d" }}>
              <CheckCircle size={16} className="shrink-0" />
              <span>Submitted {fmtDate(sub.submittedAt)}</span>
              {sub.grade != null && (
                <span className="ml-auto font-bold">{sub.grade}/{assignment.points} pts</span>
              )}
            </div>
          ) : !isAvailableFrom && !isClosed ? (
            <div className="flex items-center gap-2 px-4 py-3 rounded-lg mb-5 text-sm font-semibold"
              style={{ background: "#fef2f2", border: "1px solid #f0c0c0", color: MAROON }}>
              <Circle size={16} className="shrink-0" /> Not yet submitted
            </div>
          ) : null}

          <div
            className="cat-submitter-desc"
            dangerouslySetInnerHTML={{
              __html: assignment.description ?? '<em style="color:#9ca3af">No description provided.</em>'
            }}
          />

          {showUploadSection && (
            <div className="mt-6 border-t border-gray-100 pt-6">
              <p className="text-sm font-bold text-gray-800 mb-3">Submit Your Work</p>
              <SubmitterFileUploadSection assignment={assignment} />
            </div>
          )}
        </div>
      </div>
...
/* ─────────────────────────────────────────────────────────────────────────────
   HEAD ASSIGNMENT DETAIL (manager view)
───────────────────────────────────────────────────────────────────────────── */
function HeadAssignmentDetail({ assignment, courseId, sections, staff, onBack, onEditFull, setAssignments, currentUserId }: {
  assignment: AssignmentWithRole; courseId: string;
  sections: Section[]; staff: Staff[]; onBack: () => void;
  ...
  const downloadSubmissions = async () => {
    ...
    try {
      ...
      for (const [userId, subs] of Object.entries(byStudent)) {
        ...
        const folder = zip.folder(`${studentName}_${userId.slice(-6)}`); if (!folder) continue;
        for (let i = 0; i < subs.length; i++) {
          ...
          try {
            ...
            if (!fileName) { const urlPart = url.split("/").pop()?.split("?")[0] ?? ""; const ext = urlPart.includes(".") ? urlPart.split(".").pop() : "bin"; fileName = `submission_${i + 1}.${ext}`; }
  ...
  // ── FIX: use buildSubmittingLabel for consistent label ──
  const submittingLabel = buildSubmittingLabel(current);
  const hasDownloadable = submissions.filter(s => s.fileUrl && s.submittedAt).length > 0;
  const { hasRestrictions, allowedFileTypes, maxFiles } = getAssignmentFileRestrictions(current);
  ...
  return (
    <div className="flex flex-col h-full bg-white" style={{ fontFamily: FONT }}>
      ...
      <div className="cat-detail-header">
        ...
        <div className="cat-detail-actions">
          <button onClick={togglePublish} disabled={publishing}
            ...
            <span className="cat-action-label">{isPublished ? "Published" : "Unpublished"}</span>
          </button>
          <button onClick={() => setShowAssignPanel(true)} className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 border border-gray-200 rounded-lg hover:border-gray-400 text-gray-600 transition-all">
            <Users size={12} /> <span className="cat-action-label">Assign To</span>
          </button>
          <button onClick={() => onEditFull(current)} className="flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 border border-gray-200 rounded-lg hover:border-gray-400 text-gray-600 transition-all">
            <Pencil size={12} /> <span className="cat-action-label">Edit</span>
          </button>
      ...
      <PublisherBar
        name={current._publisherName}
        image={current._publisherImage}
        role={current._publisherRole}
        publisherId={current._publisherId}
        currentUserId={currentUserId}
      />

      <div className="cat-detail-body">
        <div className="cat-detail-main">
          <h1 className="text-xl sm:text-2xl font-black text-gray-900 mb-5">{current.title}</h1>
          <div className="mb-6">
          ...
          <div className="bg-white border rounded-lg mb-5 overflow-hidden">
            ...
            <div className="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                ...
                <p className="text-sm font-bold text-gray-800">{current.points}</p>
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Submission Type</p>
                {/* ── FIX: shows "File Upload (PDF, DOCX)" instead of generic label ── */}
                <p className="text-sm font-bold text-gray-800">{submittingLabel}</p>
              </div>
              {hasRestrictions && (
                <div className="sm:col-span-2">
                  <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 mb-1">Allowed File Types</p>
                ...
                <thead>
                  <tr className="border-b border-gray-100">
                    {["Due", "Available From", "Until"].map(h => (
                      <th key={h} className="text-left px-4 sm:px-5 py-3 text-[10px] font-black uppercase tracking-widest text-gray-400">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  <tr>
                    <td className="px-4 sm:px-5 py-3 text-sm font-semibold text-gray-700">{fmtDue(current.dueDate) || "No due date"}</td>
                    <td className="px-4 sm:px-5 py-3 text-sm text-gray-600">{current.availableFrom ? fmtDue(current.availableFrom) : "—"}</td>
                    <td className="px-4 sm:px-5 py-3 text-sm text-gray-600">{current.availableUntil ? fmtDue(current.availableUntil) : "—"}</td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <div className="cat-detail-sidebar">
          ...
          <div className="px-4 py-4 space-y-3">
            <button onClick={() => window.open(`/courses/${courseId}/assignments/${current.id}/speedgrader`, "_blank")} className="w-full flex items-center gap-2 text-xs font-bold hover:underline text-left" style={{ color: MAROON }}>
              <Zap size={13} /> SpeedGrader™
            </button>
            <button onClick={downloadSubmissions} disabled={downloading || !hasDownloadable} className="w-full flex items-center gap-2 text-xs font-bold hover:underline text-left disabled:opacity-40" style={{ color: MAROON }}>
          ...
          {submissions.length > 0 && (
            <div className="px-4 pb-4">
              <p className="text-[10px] text-gray-400 leading-relaxed">
                {submissions.filter(s => s.fileUrl && s.submittedAt).length} file{submissions.filter(s => s.fileUrl && s.submittedAt).length !== 1 ? "s" : ""} ready to download.
              </p>
            </div>
          )}
        </div>
      ...
      {showAssignPanel && (
        <AssignToPanel
          assignment={current} courseId={courseId} sections={sections} staff={staff}
          onClose={() => setShowAssignPanel(false)}
          onSave={() => {
            fetch(`/api/courses/${courseId}/assignments/${current.id}`)
              .then(r => r.json())
              .then(d => { if (d.assignment) setCurrent(p => ({ ...p, ...d.assignment })); })
              .catch(() => { });
          }}
        />
      )}
...
function SubmissionEntryCard({
  entry, index, canRemove, onRemove, onUpdate,
}: {
  ...
  const toggleFileType = (ft: string) => {
    ...
    const updated = current.includes(ft)
      ...
      <div className="px-3 py-3 space-y-3">
        <div>
          ...
          <div className="relative">
            <select
              ...
              className="w-full h-8 border border-gray-300 rounded px-2 pr-7 text-xs bg-white outline-none appearance-none focus:border-[#7b1113]"
            >
          ...
          <>
            <div>
              <label className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide block mb-2">
                Allowed File Types
                <span className="ml-1 normal-case font-normal text-gray-400">(leave empty to allow all)</span>
              </label>
              <div className="grid grid-cols-2 gap-1.5">
                {ALLOWED_FILE_TYPES.map(ft => {
                  ...
                  return (
                    <label
                      key={ft.value}
                      className={`flex items-center gap-2 px-2.5 py-1.5 rounded border cursor-pointer transition-all text-xs font-medium select-none ${
                        checked
                          ? "border-[#7b1113] bg-[#fef2f2] text-[#7b1113]"
                    ...
                    >
                      ...
                      <span
                        className={`w-3.5 h-3.5 rounded-sm border flex items-center justify-center shrink-0 transition-all ${
                          checked ? "border-[#7b1113] bg-[#7b1113]" : "border-gray-300 bg-white"
                        }`}
                      >
                        {checked && (
              ...
              {(entry.allowedFileTypes ?? []).length === 0 ? (
                ...
                <div className="mt-2 flex items-center gap-1.5 flex-wrap">
                  <span className="text-[10px] font-semibold text-gray-500">Only accepted:</span>
                  {(entry.allowedFileTypes ?? []).map(t => (
                    <span key={t} className="text-[10px] font-black px-1.5 py-0.5 rounded uppercase" style={{ background: "#fef2f2", color: MAROON, border: `1px solid #f0c0c0` }}>
                      .{t}
                    </span>
                  ))}
                </div>
            ...
            <div>
              ...
              <input
                ...
                onChange={e => onUpdate("maxFiles", Math.max(1, parseInt(e.target.value) || 1))}
                className="w-20 h-8 border border-gray-300 rounded px-2 text-xs outline-none focus:border-[#7b1113]"
              />
            </div>
          </>
        )}
        ...
        <div className="pt-1 border-t border-gray-100">
          <label className="flex items-center gap-2 cursor-pointer group">
            <div
              className={`relative w-8 h-4 rounded-full transition-colors ${entry.required ? "" : "bg-gray-200"}`}
              style={entry.required ? { background: MAROON } : {}}
              onClick={() => onUpdate("required", !entry.required)}
            >
              <div
                className={`absolute top-0.5 w-3 h-3 rounded-full bg-white shadow transition-transform ${
                  entry.required ? "translate-x-4" : "translate-x-0.5"
                }`}
              />
            </div>
            <span className="text-xs font-semibold text-gray-700">
              Required
              {entry.required && (
                <span className="ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded" style={{ background: "#fef2f2", color: MAROON }}>
                  ON
                </span>
              )}
            </span>
          ...
          {entry.required && (
            <p className="text-[10px] text-gray-400 mt-1 ml-10">
              Staff must complete this submission to submit the assignment.
            </p>
          )}
        </div>
      </div>
...
/* ─────────────────────────────────────────────────────────────────────────────
   HEAD CREATE / EDIT ASSIGNMENT
───────────────────────────────────────────────────────────────────────────── */
function HeadCreateAssignment({ courseId, initialGroup = "", existingAssignment, onCancel, onCreated }: {
  courseId: string;
  initialGroup?: string;
  ...
  const [submissionEntries, setSubmissionEntries] = useState<SubmissionEntryExtended[]>(() => {
    const existingEntries =
      (existingAssignment as AssignmentWithSubmissionEntries | null | undefined)?.submissionEntries;
  ...
  useEffect(() => {
    fetch(`/api/courses/${courseId}/assignments`).then(r => r.json()).then(d => {
      ...
      const names: string[] = [...new Set<string>(
        list
          .filter((a: AssignmentWithRole) => a._assignmentRole !== "submitter")
          .map((a: { assignmentGroup: string }) => a.assignmentGroup || "Assignments")
      )];
  ...
  const addSubmissionEntry = () => setSubmissionEntries(p => [
    ...p,
    { id: Date.now(), label: "", required: false, type: "File Upload", allowedFileTypes: [], maxFiles: 1 }
  ]);
  ...
  const updateSubmissionEntry = (
    id: number,
    field: keyof SubmissionEntryExtended,
  ...
  const handleSave = async (publish: boolean) => {
    ...
    if (submissionType === "Online" && submissionEntries.length === 0) {
      setSaveError("Please add at least one submission entry.");
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        submissionEntries: submissionEntries.map(e => ({
          ...
          required: e.required,
          type: e.type,
          allowedFileTypes: e.type === "File Upload" ? (e.allowedFileTypes ?? []) : [],
          maxFiles: e.type === "File Upload" ? (e.maxFiles ?? 1) : null,
        })),
        submissionAttempts,
      ...
      if (isEdit && existingAssignment) {
        res = await fetch(`/api/courses/${courseId}/assignments/${existingAssignment.id}`, {
          method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload),
        });
      } else {
        res = await fetch(`/api/courses/${courseId}/assignments`, {
    ...
    <div className="w-full h-full bg-white flex flex-col" style={{ fontFamily: FONT }}>
      ...
      <div className="flex items-end border-b border-gray-200 px-4 bg-white shrink-0 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
        {TABS.map(t => (
          <button key={t.key} onClick={() => setActiveTab(t.key)}
            className={`px-4 py-2 text-xs border border-b-0 -mb-px mr-0.5 rounded-t transition-colors flex-shrink-0 ${activeTab === t.key ? "bg-white border-gray-200 text-gray-900 font-medium" : "border-transparent text-gray-500 hover:text-gray-700"}`}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-5 py-5">
        ...
        {activeTab === "details" && (
          <div className="space-y-5 max-w-2xl">
            ...
            <div className="cat-create-grid">
              <label className="cat-create-label">Points</label>
              <input type="number" min={0} value={points} onChange={e => setPoints(e.target.value)} className={inp} style={{ maxWidth: 320 }} />
              <label className="cat-create-label">Assignment Group</label>
        ...
        {activeTab === "submission" && (
          <div className="max-w-2xl space-y-5">
            <div className="cat-create-grid">
              <label className="cat-create-label">Submission Type</label>
            ...
            <div className="cat-create-grid">
              <label className="cat-create-label">Submission Attempts</label>
            ...
            {/* ── SUBMISSION SUMMARY with specific file types ── */}
            {submissionType === "Online" && submissionEntries.length > 0 && (
              <div className="rounded-md p-3 border text-xs" style={{ background: "#fef2f2", borderColor: "#f0c0c0" }}>
                  ...
                  {submissionEntries.map((e) => {
                    ...
                    return (
                      <div key={e.id} className="flex items-start gap-2 bg-white rounded border border-gray-100 px-2.5 py-2">
                        ...
                        <div>
                          {/* ── FIX: show "File Upload (PDF, DOCX)" not just "File Upload" ── */}
                          <span className="font-semibold text-gray-700">
                            {e.type === "File Upload" && hasTypes
                              ? `File Upload (${(e.allowedFileTypes ?? []).map(t => t.toUpperCase()).join(", ")})`
                              : e.type}
                          </span>
                          {e.required && (
                            <span className="ml-1.5 px-1 py-0.5 rounded text-[10px] font-bold" style={{ background: "#fef2f2", color: MAROON }}>Required</span>
                          )}
                          {hasTypes && (
                            <div className="flex flex-wrap items-center gap-1 mt-1">
                              <span className="text-gray-400 text-[10px]">Only accepts:</span>
                              {(e.allowedFileTypes ?? []).map(t => (
                                <span key={t} className="px-1.5 py-0.5 rounded text-[10px] font-black uppercase" style={{ background: MAROON, color: "#fff" }}>
                                  .{t}
                                </span>
                              ))}
                              {e.maxFiles && e.maxFiles > 1 && (
                                <span className="text-gray-400 text-[10px]">· max {e.maxFiles} files</span>
                              )}
                            </div>
                          )}
                          {e.type === "File Upload" && !hasTypes && (
                            <span className="ml-1.5 text-[10px] text-gray-400">· All file types accepted</span>
                          )}
                        </div>
                      </div>
                    );
                ...
                <p className="text-gray-400 mt-2">This is what staff will see when submitting.</p>
              </div>
            )}
          </div>
        )}
        ...
        {/* ── SETTINGS TAB ── */}
        {activeTab === "settings" && (
          <div className="cat-create-grid max-w-2xl">
            <label className="cat-create-label">Display Grade as</label>
            ...
            <label className="cat-create-label">Group Assignment</label>
            <div className="border border-gray-200 rounded-sm p-3" style={{ maxWidth: 320 }}><label className="flex items-center gap-2 text-xs text-gray-700 cursor-pointer"><input type="checkbox" checked={isGroupAssignment} onChange={e => setIsGroupAssignment(e.target.checked)} style={{ accentColor: MAROON }} />This is a Group Assignment</label></div>
          </div>
        )}
        ...
        {activeTab === "assign" && (
          <div className="cat-create-grid max-w-2xl">
            <label className="cat-create-label">Assign Access</label>
            <div className="space-y-3">
                    ...
                    {configs.map(c => (
                      <div key={c.label}>
                        ...
                        <div className={`flex gap-0 border rounded-sm overflow-hidden ${c.err ? "border-red-500" : "border-gray-300"}`}>
                          ...
                          <select value={row[c.timeField]} onChange={e => updateAssignRow(row.id, c.timeField, e.target.value)} className="h-7 border-0 px-2 text-xs bg-white outline-none w-28 flex-shrink-0">
      ...
      {groupModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/20 px-4">
          <div className="w-full sm:w-[460px] bg-white shadow-xl border border-gray-200 rounded-t-2xl sm:rounded">
      ...
      <div className="shrink-0 border-t border-gray-200 bg-white px-4 sm:px-5 py-3 flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {activeTab !== "details" && <button type="button" onClick={() => setActiveTab(activeTab === "submission" ? "details" : activeTab === "settings" ? "submission" : "settings")} className="h-8 px-4 border border-gray-300 bg-white text-xs text-gray-700 rounded hover:bg-gray-50">← Back</button>}
          {activeTab !== "assign" && <button type="button" onClick={() => setActiveTab(activeTab === "details" ? "submission" : activeTab === "submission" ? "settings" : "assign")} className="h-8 px-4 border border-gray-300 bg-gray-50 text-xs text-gray-700 rounded hover:bg-gray-100">Next →</button>}
          {activeTab === "assign" && (
            <>
...
function resolveHeadAssignmentRole(
  a: AssignmentWithRole,
  currentUserId?: string | null
): "manager" | "submitter" {
  const assignedToYou =
    getBool(a._isAssignedToYou) ||
    getBool(a._isExplicitlyAssignedToYou) ||
    getBool(a.isAssignedToYou);

  const isCreator =
    getBool(a.isCreator) ||
    (!!currentUserId && !!a._publisherId && a._publisherId === currentUserId);
...
function withResolvedHeadRole(
  a: AssignmentWithRole,
  currentUserId?: string | null
): AssignmentWithRole {
  return {
    ...a,
    _assignmentRole: resolveHeadAssignmentRole(a, currentUserId),
  };
}

function HeadAssignmentList({ courseId, assignments, setAssignments, sections, staff, onViewDetail, onCreateNew, onEditFull, currentUserId }: {
  courseId: string;
  ...
  const [assignedViewMode, setAssignedViewMode] = useState<"date" | "type">("date");

  const resolvedAssignments = useMemo(
    () => assignments.map(a => withResolvedHeadRole(a, currentUserId)),
  ...
  useEffect(() => {
    ...
    const apiGroups = [...new Set(
      resolvedAssignments
        .filter(a => a._assignmentRole !== "submitter")
        .map(a => a.assignmentGroup || DEFAULT_GROUP)
    )];
    setLocalGroups(prev => {
      const merged = [...new Set([DEFAULT_GROUP, ...persisted, ...prev, ...apiGroups])];
      persistGroups(courseId, merged);
      return merged;
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [courseId, resolvedAssignments]);
  ...
  const handleDeleteGroup = (action: "delete" | "move", targetGroup?: string) => {
    ...
    if (action === "delete") {
      const toDelete = assignments.filter(a => (a.assignmentGroup || DEFAULT_GROUP) === groupName);
      ...
      assignments.filter(a => (a.assignmentGroup || DEFAULT_GROUP) === groupName).forEach(a => {
    ...
    setLocalGroups(prev => {
      if (groupName === DEFAULT_GROUP || prev.length <= 1) return prev;
  ...
  const handleQuickEditSave = async (updated: Partial<AssignmentWithRole> & { dueTime?: string }) => {
  ...
  const handleSpeedGrader = (a: AssignmentWithRole) => { window.open(`/courses/${courseId}/assignments/${a.id}/speedgrader`, "_blank"); };

  const managerAssignments = resolvedAssignments.filter(a => a._assignmentRole === "manager");
  const submitterAssignments = resolvedAssignments.filter(a => a._assignmentRole === "submitter");

  const publishedFiltered = managerAssignments.filter(a =>
    a.title.toLowerCase().includes(publishedSearch.toLowerCase())
  );
  const grouped: Record<string, AssignmentWithRole[]> = {};
  for (const g of localGroups) grouped[g] = [];
  for (const a of publishedFiltered) {
    const g = a.assignmentGroup || DEFAULT_GROUP;
    if (!grouped[g]) grouped[g] = [];
    grouped[g].push(a);
  }

  const now = new Date();
  const assignedFiltered = submitterAssignments.filter(a =>
    a.title.toLowerCase().includes(assignedSearch.toLowerCase())
  );

  const upcoming: AssignmentWithRole[] = [], undated: AssignmentWithRole[] = [], past: AssignmentWithRole[] = [];
  assignedFiltered.forEach(a => {
    if (!a.dueDate) undated.push(a);
    else if (new Date(a.dueDate) >= now) upcoming.push(a);
    else past.push(a);
  });

  const assignedTypeGroups: Record<string, AssignmentWithRole[]> = {};
  assignedFiltered.forEach(a => {
    const g = a.assignmentGroup || DEFAULT_GROUP;
    if (!assignedTypeGroups[g]) assignedTypeGroups[g] = [];
    assignedTypeGroups[g].push(a);
  });

  return (
    <div className="flex flex-col h-full" style={{ fontFamily: FONT }}>

      <div className="cat-section-label" style={{ color: MAROON, background: "#fef2f2", borderBottom: "1px solid #f0c0c0" }}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={MAROON} strokeWidth="2.5">
      ...
      <div className="cat-toolbar" style={{ borderBottom: "1px solid #e5e7eb" }}>
        <div className="relative">
        ...
        <div className="cat-toolbar-right">
          ...
          <button onClick={() => onCreateNew()} className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white rounded-lg hover:opacity-90" style={{ background: MAROON }}>
            <Plus size={14} /> <span className="hidden sm:inline">Assignment</span>
          </button>
        </div>
      </div>

      <div className="px-3 sm:px-5 py-4" style={{ borderBottom: "2px solid #e5e7eb" }}>
        {publishedFiltered.length === 0 && managerAssignments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-3">
            <p className="text-sm text-gray-400">No assignments published yet.</p>
            <button onClick={() => onCreateNew()} className="text-xs font-bold hover:underline" style={{ color: MAROON }}>
              + Create your first assignment
            </button>
          </div>
        ) : (
          Object.entries(grouped).map(([grp, items]) => (
            <AdminAssignmentGroupSection
              key={grp}
              title={grp}
              items={items}
              onAddAssignment={g => onCreateNew(g)}
              onView={a => onViewDetail(a)}
              onEdit={a => onEditFull(a)}
      ...
      <div className="cat-section-label" style={{ color: "#1d6fa4", background: "#eff6ff", borderBottom: "1px solid #bfdbfe", borderTop: "1px solid #bfdbfe" }}>
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#1d6fa4" strokeWidth="2.5">
      ...
      <div className="cat-toolbar" style={{ borderBottom: "1px solid #e5e7eb" }}>
        ...
        <div className="cat-toolbar-right">
          <div className="cat-view-toggle">
            <button className={assignedViewMode === "date" ? "active" : ""} onClick={() => setAssignedViewMode("date")}>Show by Date</button>
            <button className={assignedViewMode === "type" ? "active" : ""} onClick={() => setAssignedViewMode("type")}>Show by Type</button>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-5 py-4">
        {assignedFiltered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 gap-2">
            ...
            <p className="text-sm text-gray-400">No assignments assigned to you.</p>
          </div>
        ) : assignedViewMode === "date" ? (
          <>
            {upcoming.length > 0 && <AssignedSection title="Upcoming" items={upcoming} courseId={courseId} onView={onViewDetail} />}
            {undated.length > 0 && <AssignedSection title="Undated" items={undated} courseId={courseId} onView={onViewDetail} />}
            {past.length > 0 && <AssignedSection title="Past" items={past} courseId={courseId} onView={onViewDetail} />}
          </>
        ) : (
          <>
            {Object.entries(assignedTypeGroups).map(([grp, items]) => (
              <AssignedSection key={grp} title={grp} items={items} courseId={courseId} onView={onViewDetail} />
            ))}
          </>
        )}
      </div>
      ...
      {deleteGroupTarget && (
        <DeleteGroupModal
          ...
          assignmentCount={assignments.filter(a => (a.assignmentGroup || DEFAULT_GROUP) === deleteGroupTarget).length}
...
/* ─────────────────────────────────────────────────────────────────────────────
   ASSIGNED SECTION
───────────────────────────────────────────────────────────────────────────── */
function AssignedSection({ title, items, courseId, onView }: {
  title: string;
  items: AssignmentWithRole[];
  courseId: string;
  onView: (a: AssignmentWithRole) => void;
          ...
          {items.map((a, idx) => {
            ...
            return (
              <div
                key={a.id}
                className="cat-row"
                style={{ borderBottom: idx < items.length - 1 ? "1px solid #e5e7eb" : "none" }}
                onClick={() => onView(a)}
              >
                <div className="w-[3px] h-8 rounded-full shrink-0" style={{ background: "#60a5fa" }} />
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-800 truncate">{a.title}</p>
                  <div className="cat-row-meta text-xs text-gray-500 mt-0.5">
                    {isLocked && <span className="font-medium text-amber-600">Opens {fmtDate(a.availableFrom)}</span>}
                    {isClosed && <span className="font-medium text-gray-500">Closed</span>}
                    {due && <><span><span className="font-medium text-gray-700">Due</span> {due}</span><span>·</span></>}
                    <span>{a.points} pts</span>
                    {sub?.submittedAt && (
                      <><span>·</span>
                    ...
                    {a._publisherName && (
                      <><span>·</span>
                        <PublisherChip name={a._publisherName} image={a._publisherImage} />
                      </>
                    )}
                  </div>
                </div>
...
function StudentAssignmentRow({ a, courseId }: { a: AssignmentWithRole; courseId: string }) {
  const router = useRouter();
  const now = new Date();
  const sub = a.submissions?.[0];
  const isLocked = a.availableFrom && now < new Date(a.availableFrom);
  const isClosed = a.availableUntil && now > new Date(a.availableUntil);
  return (
    <div className="cat-row" onClick={() => router.push(`/courses/${courseId}/assignments/${a.id}`)}>
      ...
      <div className="flex-1 min-w-0">
        <button className="text-sm font-semibold text-left truncate block w-full" style={{ color: COLORS.primary, fontFamily: FONT }}>{a.title}</button>
        <div className="cat-row-meta text-xs text-gray-500 mt-0.5">
          ...
          {a._publisherName && <><span>·</span><PublisherChip name={a._publisherName} image={a._publisherImage} /></>}
        </div>
      </div>
    </div>
  );
...
function StudentAssignmentSection({ title, items, courseId }: { title: string; items: AssignmentWithRole[]; courseId: string }) {
  const [collapsed, setCollapsed] = useState(false);
  return (
    <div>
...
───────────────────────────────────────────────────────────────────────────── */
function StaffAssignmentList({ assignments, courseId }: { assignments: AssignmentWithRole[]; courseId: string }) {
  const [search, setSearch] = useState("");
  const [viewMode, setViewMode] = useState<"name" | "type">("name");
  ...
  const typeGroups: Record<string, AssignmentWithRole[]> = {};
  filtered.forEach(a => {
    const g = a.assignmentGroup || DEFAULT_GROUP;
      ...
      <div className="cat-toolbar">
        <div className="relative">
          ...
          <input placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}
            className="pl-9 pr-4 py-1.5 border rounded text-sm w-44 sm:w-52 focus:outline-none"
            style={{ borderColor: "#d1d5db", fontFamily: FONT }} />
        </div>
        <div className="cat-toolbar-right">
          <div className="cat-view-toggle">
            <button className={viewMode === "name" ? "active" : ""} onClick={() => setViewMode("name")}>Show by Name</button>
            <button className={viewMode === "type" ? "active" : ""} onClick={() => setViewMode("type")}>Show by Type</button>
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <>
          {Object.entries(typeGroups).map(([grp, items]) => (
            <StudentAssignmentSection key={grp} title={grp} items={items} courseId={courseId} />
          ))}
        </>
      )}
    </div>
...
export default function CourseAssignmentsTab({
  courseId, assignments: rawAssignments, setAssignments: rawSetAssignments,
  sections, staff, isHead, canManageAssignments, currentUserId,
}: Props) {
  ...
  const [assignViewTarget, setAssignViewTarget] = useState<AssignmentWithRole | null>(null);
  ...
  if (isHead && canManageAssignments) {
    ...
    if (assignView === "detail" && assignViewTarget) {
      if (assignViewTarget._assignmentRole === "submitter") {
        return (
          <>
            <style>{RESPONSIVE_CSS}</style>
            <HeadSubmitterDetail
              assignment={assignViewTarget}
              courseId={courseId}
              currentUserId={currentUserId}
      ...
      return (
        <>
          ...
          <HeadAssignmentDetail
            ...
            onBack={() => { setAssignView("list"); setAssignViewTarget(null); }}
            onEditFull={a => {
              setAssignViewTarget({ ...a, _assignmentRole: "manager" });
              setAssignView("edit");
            }}
      ...
      return (
        <>
          ...
          <HeadCreateAssignment
            ...
            existingAssignment={assignViewTarget}
            onCancel={() => { setAssignView("detail"); }}
  ...
  return (
    <>
      <style>{RESPONSIVE_CSS}</style>
      <StaffAssignmentList assignments={assignments} courseId={courseId} />
    </>
  );
}