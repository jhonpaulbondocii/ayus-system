...
interface Chip { id: string; email: string; role: string; status: "idle"|"valid"|"invalid"; errorMsg?: string; }
...
const SpinnerInput = ({ value, onChange, placeholder, width=160 }: { value:number; onChange:(v:number)=>void; placeholder?:string; width?:number; }) => (
  <div className="cpt-spin-wrap" style={{ width }}>
    <input type="number" min={0} value={value||""} onChange={e=>onChange(parseInt(e.target.value)||0)} placeholder={placeholder??"No limit"} className="cpt-spin-input"/>
...
function Avatar({ name, image, size=32 }: { name:string|null; image:string|null; size?:number }) {
  if (image) return <img src={image} alt={name??""} style={{ width:size, height:size, borderRadius:"50%", objectFit:"cover", display:"block", flexShrink:0 }}/>;  // eslint-disable-line @next/next/no-img-element
...
const RolePills = ({ raw }: { raw: string }) => (
  <span style={{ display:"inline-flex", gap:4, flexWrap:"wrap" }}>
    {parseRoles(raw).map(r => (
      <span key={r} className="cpt-badge" style={{ background:ROLE_BADGE[r].bg, color:ROLE_BADGE[r].color }}>{r}</span>
    ))}
  </span>
);
...
export default function CoursePeopleTab({
  ...
  const fetchGroupSets = useCallback(()=>{
    fetch(`/api/courses/${courseId}/groupsets`).then(r=>r.json()).then(d=>setGroupSets(d.groupSets??[])).catch(()=>{});
  },[courseId]);

  const fetchGroupSetsAsync = useCallback((): Promise<void> =>
    new Promise(resolve=>{ fetch(`/api/courses/${courseId}/groupsets`).then(r=>r.json()).then(d=>{setGroupSets(d.groupSets??[]);resolve();}).catch(()=>resolve()); })
  ,[courseId]);

  useEffect(()=>{ fetchGroupSets(); },[fetchGroupSets]);

  const fetchPeople = useCallback(()=>{
    fetch(`/api/courses/${courseId}/people`).then(r=>r.json())
      .then(d=>setPeople((d.people??[]).map((p:Person)=>({...p,role:p.role}))))
      .catch(()=>{});
  },[courseId]);
  ...
  const handleSaveRoles = async () => {
    ...
    setSavingRoles(true);
    try {
      const res = await fetch(`/api/courses/${courseId}/people/${editRolesTarget.id}`, {
  ...
  const handleSaveOffice = async () => {
    ...
    try {
      const res = await fetch(`/api/courses/${courseId}/people/${changeOfficeTarget.id}`, {
        method: "PATCH", headers: { "Content-Type": "application/json" },
  ...
  const commitInput = () => {
    ...
    if (!already) setChips(prev => [...prev, { id: crypto.randomUUID(), email: v, role: serializeRoles(inputRoles), status: "idle" }]);
    setInputVal("");
  };
  const removeChip = (id: string) => setChips(p => p.filter(c => c.id !== id));
  const handleKD = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if ((e.key === "Enter" || e.key === ",") && inputVal.trim()) { e.preventDefault(); commitInput(); }
    if (e.key === "Backspace" && inputVal === "" && chips.length > 0) setChips(p => p.slice(0,-1));
  };
  ...
  const handleAddPeople = async () => {
    ...
    if (currentChips.length === 0) { setAddError("Please enter at least one email address."); return; }
    const badFormat = currentChips.filter(c => !isValidEmailFormat(c.email));
    if (badFormat.length > 0) { setChips(currentChips.map(c => badFormat.find(b=>b.id===c.id) ? { ...c, status: "invalid", errorMsg: "Invalid email format" } : c)); setAddError(`Invalid email format: ${badFormat.map(b=>b.email).join(", ")}`); return; }
    setAdding(true); setAddError(""); setChips(currentChips.map(c=>({...c,status:"idle"})));
  ...
  const handleEditGs = async () => { if(!editGsModal||!editGsName.trim()) return; setSavingEditGs(true); try{ const res=await fetch(`/api/courses/${courseId}/groupsets/${editGsModal.id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:editGsName.trim(),selfSignUp:editGsSelfSignUp,requireSameSection:editGsRequireSameSection,autoAssignLeader:editGsAutoAssignLeader,leaderType:editGsLeaderType,limitGroupMembers:editGsLimit})}); if(res.ok){await fetchGroupSetsAsync();setActiveTab(editGsModal.id);setEditGsModal(null);} }finally{setSavingEditGs(false);} };
  const handleCloneGs = async () => { if(!cloneModal) return; setSubmittingClone(true); try{ const res=await fetch(`/api/courses/${courseId}/groupsets`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:cloneName.trim()||`(Clone) ${cloneModal.name}`,selfSignUp:cloneModal.selfSignUp,requireSameSection:cloneModal.requireSameSection,groupStructure:cloneModal.groupStructure,createGroupsNow:0,limitGroupMembers:cloneModal.limitGroupMembers,autoAssignLeader:cloneModal.autoAssignLeader,leaderType:cloneModal.leaderType})}); if(res.ok){const d=await res.json();const newId=d.groupSet?.id;for(const g of cloneModal.groups){await fetch(`/api/courses/${courseId}/groupsets/${newId}/groups`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:g.name,membershipLimit:0})});}await fetchGroupSetsAsync();if(newId)setActiveTab(newId);}setCloneModal(null);}finally{setSubmittingClone(false);} };
  const handleAddGroup = async () => { if(!newGroupName.trim()||!addGroupModal) return; setSavingGroup(true); try{ const res=await fetch(`/api/courses/${courseId}/groupsets/${addGroupModal}/groups`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:newGroupName.trim(),membershipLimit:newGroupLimit})}); if(res.ok){fetchGroupSets();setAddGroupModal(null);setNewGroupName("");setNewGroupLimit(0);} }finally{setSavingGroup(false);} };
  ...
  const deleteGroup = async (gsId:string,gId:string) => { await fetch(`/api/courses/${courseId}/groupsets/${gsId}/groups/${gId}`,{method:"DELETE"}); fetchGroupSets(); setGroupMenuOpen(null); };
  const addStudentToGroup = async (gsId:string,gId:string,userId:string) => { setAddToGroupMenu(null); await fetch(`/api/courses/${courseId}/groupsets/${gsId}/groups/${gId}/members`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId})}); fetchGroupSets(); };
  const removeMember = async (gsId:string,gId:string,userId:string) => { await fetch(`/api/courses/${courseId}/groupsets/${gsId}/groups/${gId}/members/${userId}`,{method:"DELETE"}); fetchGroupSets(); setMemberMenuOpen(null); };
      ...
      <div className="cpt-root">
        ...
        <div className="cpt-tabbar">
          <div className="cpt-tabs">
            {tabs.map(t=>(
              <button key={t.key} className={`cpt-tab${activeTab===t.key?" active":""}`} onClick={()=>setActiveTab(t.key)}>{t.label}</button>
            ))}
          </div>
          {canManagePeople&&(
          ...
          <>
            ...
            <div className="cpt-groups-layout">
              <div className="cpt-unassigned-col">
                ...
                {unassigned.length===0?<div className="cpt-box-dashed cpt-empty-box">No unassigned members.</div>:(
          ...
          <div className="cpt-overlay" onClick={e=>{if(e.target===e.currentTarget)setEditRolesTarget(null);}}>
            <div className="cpt-modal" style={{width:420}}>
              ...
              <div className="cpt-modal-body">
                ...
                <div className="cpt-role-check-row">
                  {ALL_ROLES.map(r=>{
                    ...
                    return(
                      <label key={r} className={`cpt-role-check-item${checked?" selected":""}`}
                        ...
                        <input type="checkbox" checked={checked} disabled={disabled}
                          onChange={()=>!disabled&&toggleRoleCheck(r)}
                          style={{accentColor:"#7b1113",marginTop:2,width:16,height:16,flexShrink:0,cursor:disabled?"not-allowed":"pointer"}}/>
                        <div>
                          <span style={{fontWeight:700,fontSize:13,color:checked?ROLE_BADGE[r].color:"#2d3b45"}}>{r}</span>
                          <p className="cpt-role-desc">{desc}</p>
                          {disabled&&<p style={{fontSize:11,color:"#c0392b",marginTop:2,fontWeight:600}}>Only Admin can assign this role.</p>}
                        </div>
                      </label>
                    );
                  })}
                </div>
                {editRolesChecked.length===0&&<p style={{fontSize:12,color:"#c0392b",marginTop:10,fontWeight:600}}>At least one role must be selected.</p>}
              </div>
              <div className="cpt-modal-footer">
                <button className="cpt-btn-secondary" onClick={()=>setEditRolesTarget(null)}>Cancel</button>
                <button className="cpt-btn-primary" onClick={()=>void handleSaveRoles()} disabled={savingRoles||editRolesChecked.length===0}>
                  {savingRoles&&<Loader2 size={13} className="cpt-spin"/>}Save Roles
                </button>
              </div>
            </div>
          </div>
        ...
        {changeOfficeTarget&&canManagePeople&&(
          <div className="cpt-overlay" onClick={e=>{if(e.target===e.currentTarget)setChangeOfficeTarget(null);}}>
            <div className="cpt-modal" style={{width:440}}>
              ...
              <div className="cpt-modal-body">
                ...
                {loadingCourses?(
                  <div style={{display:"flex",alignItems:"center",gap:8,padding:"14px 0",color:"#9ca3af",fontSize:13}}>
                    <Loader2 size={16} className="cpt-spin"/>Loading offices...
                  </div>
                ):(
                  <div style={{position:"relative"}}>
              ...
              <div className="cpt-modal-footer">
                <button className="cpt-btn-secondary" onClick={()=>setChangeOfficeTarget(null)}>Cancel</button>
                <button className="cpt-btn-primary" onClick={()=>void handleSaveOffice()} disabled={savingOffice||!selectedNewCourse}>
                  {savingOffice&&<Loader2 size={13} className="cpt-spin"/>}Transfer
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="cpt-overlay" onClick={e=>{if(e.target===e.currentTarget)closeAddModal();}}>
            <div className="cpt-modal" style={{width:520}}>
              ...
              <div className="cpt-modal-body" style={{display:"flex",flexDirection:"column",gap:18}}>
                ...
                <div>
                  <label style={{display:"block",fontSize:13,fontWeight:700,color:"#2d3b45",marginBottom:6}}>Email Addresses <span style={{color:"#c0392b"}}>*</span></label>
                  <div className="cpt-chip-box" onClick={()=>inputRef.current?.focus()}>
                    {chips.map(chip=>{
                      const isInvalid=chip.status==="invalid"; const isValid=chip.status==="valid";
                      return(
                        <span key={chip.id} title={chip.errorMsg} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"3px 6px 3px 8px",borderRadius:6,fontSize:12,background:isValid?"#f0fdf4":isInvalid?"#fef2f2":"#f3f4f6",border:`1px solid ${isValid?"#86efac":isInvalid?"#f0c0c0":"#e5e7eb"}`,color:isValid?"#15803d":isInvalid?"#7b1113":"#374151"}}>
                          ...
                          <button type="button" onMouseDown={e=>{e.preventDefault();removeChip(chip.id);}} style={{background:"none",border:"none",cursor:"pointer",padding:0,display:"flex",color:"inherit",opacity:0.6,marginLeft:1}}><X size={11}/></button>
                        </span>
                      );
                    })}
                    <input ref={inputRef} className="cpt-chip-input" type="email" value={inputVal} onChange={e=>setInputVal(e.target.value)} onKeyDown={handleKD} onPaste={handlePaste} placeholder={chips.length===0?"e.g. juan@example.com":""} autoComplete="off"/>
                  </div>
...
{inputRoles.length===0&&<p style={{fontSize:12,color:"#c0392b",marginTop:4,fontWeight:600}}>At least one role required.</p>}
              ...
              <div className="cpt-modal-footer">
                ...
                <button className="cpt-btn-primary" onClick={()=>void handleAddPeople()} disabled={adding||(chips.length===0&&!inputVal.trim())}>
                  {adding&&<Loader2 size={13} className="cpt-spin"/>}{adding?"Adding…":`Add ${validChipCount>0?validChipCount:""} to Course`}
                </button>
              </div>
            </div>
          </div>
