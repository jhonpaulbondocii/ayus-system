...
import {
  ...
  Inbox, MailOpen, Clock, ArrowUpRight, Archive, UserRound,
  BookOpen, ClipboardList, Megaphone, GraduationCap, FileText,
  ArrowLeft, RefreshCw, ChevronRight, Bell, Tag, Circle,
} from "lucide-react";
...
@media (max-width: 900px) {
  ...
  .ibx-detail-panel { position: fixed !important; inset: 0 !important; z-index: 200 !important; }
}
@media (max-width: 600px) {
  .ibx-compose-modal { max-width:100% !important; width:100% !important; height:100% !important; max-height:100% !important; border-radius:0 !important; }
...
// ── Nav Item (lifted outside PersonPickerDropdown to fix static-components lint) ─
...
// ── Person Row (lifted outside PersonPickerDropdown) ───────────────────────────
...
function PersonPickerDropdown({
  ...
  // Fix: wrap setState in async function, not synchronously in effect body
  useEffect(() => {
  ...
  // Fix: derive people from allPeople reactively without a synchronous setState in effect
        ...
        <div style={{ padding: "14px 20px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <h2 style={{ fontSize: 16, fontWeight: 700, color: C.text, lineHeight: 1.3 }}>{item.subject}</h2>
            </div>
          </div>
        </div>

        {/* Body */}
        <div className="ibx-scroll" style={{ flex: 1, overflowY: "auto", padding: 24 }}>
              ...
              return (
                <div>
                  <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
                    {a.dueDate && (
                      <DetailChip label="Due" value={fmtDue(a.dueDate) ?? ""} />
                    )}
    ...
    <div className="ibx-detail-panel">
      ...
      <div style={{ padding: "12px 16px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
          <button className="ibx-btn ibx-icon-btn" onClick={onBack}><ArrowLeft size={15} /></button>
          <h2 style={{ flex: 1, fontSize: 15, fontWeight: 700, color: C.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            {convo.subject}
          </h2>
      ...
      {/* Messages */}
      <div className="ibx-scroll" style={{ flex: 1, overflowY: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 20, background: C.bg }}>
        {convo.messages.map(msg => {
          ...
          return (
            <div key={msg.id} style={{ display: "flex", flexDirection: isMine ? "row-reverse" : "row", gap: 10, alignItems: "flex-end", animation: "fadeIn 0.2s ease" }}>
              ...
              <div style={{ maxWidth: "66%", display: "flex", flexDirection: "column", gap: 3, alignItems: isMine ? "flex-end" : "flex-start" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
...
function ComposeModal({ initialRecipient, courseOptions, onClose, onSent }: {
  ...
  return (
    <div className="ibx-compose-overlay"
      style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.35)", backdropFilter: "blur(3px)", padding: 16 }}>
      <div className="ibx-compose-modal"
        style={{ background: C.surface, borderRadius: 12, width: "100%", maxWidth: 540, maxHeight: "88vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.15)", overflow: "hidden", animation: "scaleIn 0.18s ease" }}>

        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", background: C.maroon, borderBottom: `1px solid ${C.maroonDark}` }}>
        ...
        {/* Footer */}
        <div style={{ display: "flex", alignItems: "center", padding: "10px 18px", borderTop: `1px solid ${C.border}`, background: C.surfaceAlt, gap: 8 }}>
...
// ── Sidebar ────────────────────────────────────────────────────────────────────
function Sidebar({
  mailbox, category, onMailbox, onCategory, unreadCount, courses, selectedCtx, onSelectCtx,
}: {
  ...
  return (
    <div className="ibx-sidebar">
      ...
      <div style={{ padding: "16px 14px 12px", borderBottom: `1px solid ${C.border}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 26, height: 26, borderRadius: 7, background: C.maroon, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Inbox size={13} color="#fff" />
          </div>
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, color: C.text, fontFamily: FONT }}>Inbox</p>
            <p style={{ fontSize: 10, color: C.textMuted, fontFamily: FONT_MONO }}>Messaging Center</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "10px 0" }} className="ibx-scroll">
        {/* Mailbox */}
        <p style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.1em", padding: "0 14px 6px" }}>Mailbox</p>
        {MAILBOX_FILTERS.map(({ key, label }) => (
          <button key={key} className={`ibx-sidebar-item ${mailbox === key ? "active" : ""}`}
            onClick={() => onMailbox(key)}>
            {label}
            {key === "inbox" && unreadCount > 0 && (
              <span style={{ marginLeft: "auto", fontSize: 10, fontWeight: 800, background: C.maroon, color: "#fff", borderRadius: 10, padding: "1px 6px", fontFamily: FONT_MONO }}>
                {unreadCount}
              </span>
            )}
          </button>
        ...
        {/* Categories */}
        <p style={{ fontSize: 10, fontWeight: 700, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.1em", padding: "14px 14px 6px" }}>Categories</p>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {coursesOpen && (
            <div style={{ animation: "fadeIn 0.15s ease" }}>
              <button className={`ibx-sidebar-item ${!selectedCtx ? "active" : ""}`} onClick={() => onSelectCtx(null)}>
                All Courses
              </button>
...
export default function InboxPage({ currentUserId: propUserId }: { currentUserId: string }) {
      ...
      if (convRes.status === "fulfilled") {
        ...
        for (const c of convos) {
          items.push({
            ...
            senderImage: c.participants[0]?.image,
            data:        c,
          });
        }
      }
      ...
      if (qzRes.status === "fulfilled") {
        ...
        for (const q of quizzes) {
          items.push({
            ...
            date:        q.dueDate ?? new Date().toISOString(),
            unread:      !q.attempted,
            sender:      q.course?.name ?? "Quiz",
            courseColor: q.course?.color,
        ...
        for (const f of forms) {
          items.push({
            ...
            unread:      !f.submitted,
            sender:      f.course?.name ?? "Form",
            courseColor: f.course?.color,
  ...
  return (
    <div className="ibx-root">
      ...
      {/* Top bar */}
      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderBottom: `1px solid ${C.border}`, background: C.surface, flexShrink: 0, zIndex: 10 }}>
        {/* Search */}
        <div
          style={{ flex: 1, maxWidth: 520, display: "flex", alignItems: "center", gap: 8, background: C.maroonBg, border: `1px solid ${C.maroonBorder}`, borderRadius: 8, padding: "0 12px", height: 36, transition: "border-color 0.15s" }}
          onFocusCapture={e => (e.currentTarget.style.borderColor = C.maroon)}
        ...
        >
          ...
          />
          {searchQuery && (
            <button className="ibx-btn" onClick={() => setSearchQuery("")} style={{ color: C.textMuted, width: 14, height: 14 }}>
              <X size={12} />
            </button>
          )}
        </div>
        ...
        <button className="ibx-btn"
          ...
          style={{ padding: "0 16px", height: 36, borderRadius: 8, background: C.maroon, color: "#fff", border: "none", fontSize: 13, fontWeight: 600, gap: 7, flexShrink: 0, position: "relative" }}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <span style={{ fontSize: 13, fontWeight: 700, color: C.text }}>
                  {CATEGORY_FILTERS.find(f => f.key === category)?.label ?? "All Mail"}
                </span>
                {selectedCtx && (
